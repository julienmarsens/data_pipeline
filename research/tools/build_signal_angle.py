...
# ───────────────── main driver ─────────────────────────────────────
def loop_over_pairs(pairs, rolling_angle_windows_in_days, fitting_dates=None):
    """
    ...
    with PdfPages(out_pdf) as pdf:
        for _, sigma, r2o, (a1, a2), df, highlight in rec:
            fig = make_signal_angle_figure(df, f'{a1} – {a2}', rolling_angle_windows_in_days, thresh_deg=10, highlight=highlight)
            pdf.savefig(fig)
            plt.close(fig)
...
if __name__ == '__main__':
    ...
    # Example with highlight (and fitting-period angle on chart 3):
    # loop_over_pairs(
    #     [('ADAUSDT', 'DOGEUSDT')],
