...
class BacktestingEngine:
    ...
    def __init__(self, merged_df, entry_threshold=0.0005, exit_threshold=0.0001,
        ...
        self.entry_threshold = 0.00002
        self.exit_threshold = 0.000005
        ...
        for i in range(len(self.df)):
            ...
            if i % self.rebalance_period == 0:
                ...
                if ecm_signal > self.entry_threshold:
                    ...
                    current_pos_2 = -qty_2
                    signal = 1
                    print(f"[{i}] LONG spread  | ecm_signal={ecm_signal:.6f}")
                elif ecm_signal < -self.entry_threshold:
                    current_pos_1 = -qty_1
                    current_pos_2 = qty_2
                    signal = -1
                    print(f"[{i}] SHORT spread | ecm_signal={ecm_signal:.6f}")
                elif abs(ecm_signal) < self.exit_threshold:
                    current_pos_1 = 0
                    current_pos_2 = 0
                    signal = 0
                    print(f"[{i}] EXIT         | ecm_signal={ecm_signal:.6f}")
