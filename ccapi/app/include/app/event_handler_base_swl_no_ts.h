...
*/
    ...
    if (eventType == Event::Type::SUBSCRIPTION_DATA) {
      ...
      //            firstMessage.getType() == Message::Type::CANCEL_ORDER)) {
        ...
        // TODO: REMOVE IN LOGGER INFO this->openBuyOrder_a.get().orderId IF this->openBuyOrder_a DOESN T EXIST...SEE BELOW
...
*/
  ...
  */
  ...
  std::string previousSamplingSec{}, currentSamplingSec{};
  //
 ...
 protected:
  ...
  virtual void postProcessMessageMarketDataEventMarketDepth(const Event& event, Session* session, std::vector<Request>& requestList, const Message& message, const TimePoint& messageTime) {
    ...
    this->currentSamplingSec = messageTimeISO.substr(24, 1);

    if(this->previousSamplingSec.empty()  || this->previousSamplingSec != this->currentSamplingSec) {
    ...
    */
          ...
          if(this->instrument_type_b != "INVERSE") {
            if(!isContractDenominated_b) {
              this->orderAmount_b = this->typicalOrderSize/midPriceA;
            } else {
              this->orderAmount_b = (this->typicalOrderSize/midPriceA)/std::stod(this->orderQuantityIncrement_b);
            }
          } else {
            if(!isContractDenominated_b) {
    ...
    */
          ...
          APP_LOGGER_DEBUG("##########################################################");
          APP_LOGGER_DEBUG("# Risk wall/limit breach status: if abs(exposure) > max inventory then risk wall is reach for that direction.");
          ...
          if(this->instrument_type_a != "INVERSE") {
            if(!isContractDenominated_a) {
              upperLimitReached_a = -this->maxInventory >= this->quoteBalance_a - this->orderAmount_a * midPriceA;
              lowerLimitReached_a = this->maxInventory <= this->quoteBalance_a + this->orderAmount_a * midPriceA;
              ...
              upperLimitReached_a = -this->maxInventory >= this->quoteBalance_a - this->orderAmount_a * midPriceA * std::stod(this->orderQuantityIncrement_a);
              lowerLimitReached_a = this->maxInventory <= this->quoteBalance_a + this->orderAmount_a * midPriceA * std::stod(this->orderQuantityIncrement_a);
            ...
            if(!isContractDenominated_a) {
              upperLimitReached_a = -this->maxInventory >= this->quoteBalance_a - this->orderAmount_a;
              lowerLimitReached_a = this->maxInventory <= this->quoteBalance_a + this->orderAmount_a;
              ...
              upperLimitReached_a = -this->maxInventory >= this->quoteBalance_a - this->orderAmount_a * std::stod(this->orderQuantityIncrement_a);
              lowerLimitReached_a = this->maxInventory <= this->quoteBalance_a + this->orderAmount_a * std::stod(this->orderQuantityIncrement_a);  

              APP_LOGGER_DEBUG("# [prod_a] -> Lower new exposure : " + std::to_string(this->quoteBalance_a + this->orderAmount_a * std::stod(this->orderQuantityIncrement_a)));
              APP_LOGGER_DEBUG("# [prod_a] -> Upper new exposure : " + std::to_string(this->quoteBalance_a - this->orderAmount_a * std::stod(this->orderQuantityIncrement_a)));       
            }
          }
          ...
          if(this->instrument_type_b != "INVERSE") {
            if(!isContractDenominated_b) {
              upperLimitReached_b = -this->maxInventory >= this->quoteBalance_b - this->orderAmount_b * midPriceB;
              lowerLimitReached_b = this->maxInventory <= this->quoteBalance_b + this->orderAmount_b * midPriceB;
              ...
              upperLimitReached_b = -this->maxInventory >= this->quoteBalance_b - this->orderAmount_b * midPriceA * std::stod(this->orderQuantityIncrement_b);
              lowerLimitReached_b = this->maxInventory <= this->quoteBalance_b + this->orderAmount_b * midPriceA * std::stod(this->orderQuantityIncrement_b);
            ...
            if(!isContractDenominated_b) {
              upperLimitReached_b = -this->maxInventory >= this->quoteBalance_b - this->orderAmount_b;
              lowerLimitReached_b = this->maxInventory <= this->quoteBalance_b + this->orderAmount_b;
              ...
              upperLimitReached_b = -this->maxInventory >= this->quoteBalance_b - this->orderAmount_b * std::stod(this->orderQuantityIncrement_b);
              lowerLimitReached_b = this->maxInventory <= this->quoteBalance_b + this->orderAmount_b * std::stod(this->orderQuantityIncrement_b);  

              APP_LOGGER_DEBUG("# [prod_b] -> Lower new exposure : " + std::to_string(this->quoteBalance_b + this->orderAmount_b * std::stod(this->orderQuantityIncrement_b)));
              APP_LOGGER_DEBUG("# [prod_b] -> Upper new exposure : " + std::to_string(this->quoteBalance_b - this->orderAmount_b * std::stod(this->orderQuantityIncrement_b)));       
            }
          }
        ...
        this->currentSamplingSec = this->previousSamplingSec;
      }
    } // sampler to sec

    // eventually rebalance portfolio to prevent long term misalignment
    // NOT USED AS MARKET CHASER BUT ONLY FOR BALANCE RETRIEVAL (SEE. TOP FUNCTION CONDITIONS)
  ...
  virtual void postProcessMessageMarketDataEventPrivateTrade(const Event& event, Session* session, std::vector<Request>& requestList, const Message& message) {
    ...
    for (const auto& element : message.getElementList()) {
      ...
      // misalignment in USD equivalent -> opposite sign then the sum should be == 0 if cash neutrality
