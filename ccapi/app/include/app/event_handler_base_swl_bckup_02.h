...
//long previousCrossingMicro{}; = getUnixTimestampMicro(tpnow());
    ...
    */
    ...
    if(this->isAllSubscriptionStarted) {
              ...
              (!this->isStopLossTriggered && (this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER))) {
                ...
                if(this->isAccountBalanceUpdated_a && this->isAccountBalanceUpdated_b) {
                  ...
                  if(std::stod(this->orderQuantityIncrement_a)<std::stod(this->orderQuantityIncrement_b)) {
                    ...
                    // deal with leg b as the min order size b is smaller than the one from leg a 
                    if(abs(abs(this->quoteBalance_b)-abs(this->quoteBalance_a * this->normalizedTradingVector_0/this->normalizedTradingVector_1))>=std::stod(this->orderQuantityIncrement_b)) {
                      
                      // amounts expressed in USD
                      double targetPosition_b=-this->quoteBalance_a * abs(this->normalizedTradingVector_0/this->normalizedTradingVector_1);
                      ...
                      this->placeQuote(event, session, requestList, tpnow(), 0.0, abs(orderSize_b), 1, orderSize_b>0?0:1, true);
                    }

                  } else {
                    
                    // deal with leg a as the min order size a is smaller than the one from leg b 
                    if(abs(abs(this->quoteBalance_a)-abs(this->quoteBalance_b * this->normalizedTradingVector_1/this->normalizedTradingVector_0))>=std::stod(this->orderQuantityIncrement_a)) {
                      
                      // amounts expressed in USD
                      double targetPosition_a=-this->quoteBalance_b * abs(this->normalizedTradingVector_1/this->normalizedTradingVector_0);
                      ...
                      // send a market order (no need for quote level) to align position on leg b
                      this->placeQuote(event, session, requestList, tpnow(), 0.0, abs(orderSize_a), 0, orderSize_a>0?0:1, true);
                    }
                  }
                   
                }
              } // END REBALANCE
            } // END LIQUIDATION & REBALANCE
/*
