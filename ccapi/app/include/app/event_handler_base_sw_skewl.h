...
class EventHandlerBase : public EventHandler {
 public:
  ...
  virtual void onInit(Session* session) {
    ...
    | skewLowerBound   | float(10,5)  | YES  |     | 1.00000           |                   |
    | skewUpperBound   | float(10,5)  | YES  |     | 1.00000           |                   |
    +------------------+--------------+------+-----+-------------------+-------------------+
    */
    ...
    while (res->next()) {
      ...
      this->skewLowerBound = res->getDouble("skewLowerBound");    
      this->skewUpperBound = res->getDouble("skewUpperBound");    
    }

    if(!isStateExist) {
      // is state doesn't exist for that trader then create it.
      pstmt_3 = con->prepareStatement("INSERT INTO states(TraderId, BaseBalanceA, BaseBalanceB, QuoteBalanceA, QuoteBalanceB, TheoreticalPrice, skewLowerBound, skewUpperBound) VALUES (?,?,?,?,?,?,?,?)");
      ...
      pstmt_3->setDouble(7, 1.0);
      pstmt_3->setDouble(8, 1.0);
    ...
    if(isStateExist && this->isReinit) {
      ...
      pstmt_3 = con->prepareStatement("UPDATE states SET BaseBalanceA=?, BaseBalanceB=?, QuoteBalanceA=?, QuoteBalanceB=?, TheoreticalPrice=?, skewLowerBound=?, skewUpperBound=? WHERE TraderId='"+this->trader_id+"';");     
    ...
    pstmt_2 = con->prepareStatement("UPDATE states SET BaseBalanceA=?, BaseBalanceB=?, QuoteBalanceA=?, QuoteBalanceB=?, TheoreticalPrice=?, skewLowerBound=?, skewUpperBound=? WHERE TraderId='"+this->trader_id+"';");
    ...
    */
    ...
    if (eventType == Event::Type::SUBSCRIPTION_DATA) {
      ...
      if (index != -1) {
       ...
       if(!this->isLiquitation && !this->isStopLossTriggered && ((this->isAllSubscriptionStarted && (!(this->isReinit || this->isTarget) || (this->isLiquidated_a && this->isLiquidated_b))))){ 
       
       
 
        //if(!this->isStopLossTriggered && ((this->isAllSubscriptionStarted && (!(this->isReinit || this->isTarget) || (this->isLiquidated_a && this->isLiquidated_b))) || this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER)) {
          ...
          if(instrument_name==this->instrument_a || instrument_name==this->instrument_b) {
            ...
            if(this->isReinit && this->isAllSubscriptionStarted) {
              ...
              if(productId=="prodA" && !this->isLiquidationOrderSent_a) {
                ...
                if(amountTaker_a !=0.0 && !this->isLiquidationOrderSent_a) {
                  ...
                  if(this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER) {
                    ...
                    aSellLevelTakerA = std::stod(this->bestBidPrice_a) - this->offset * abs(std::stod(this->bestBidPrice_a) - std::stod(this->bestAskPrice_a)); // sell into the order book == mkt order : BEWARE OF THE DUST...

                    // send a buy mkt order
                    aBuyLevelTakerA = std::stod(this->bestAskPrice_a) + this->offset * abs(std::stod(this->bestBidPrice_a) - std::stod(this->bestAskPrice_a)); // buy into the order book == mkt order : BEWARE OF THE DUST...
                  }
                  ...
                  //this->placeQuote(event, session, requestList, tpnow(), this->exchangePositionUSD_a>0?aSellLevelTakerA:aBuyLevelTakerA, amountTaker_a, productId=="prodA"?0:1, this->exchangePositionUSD_a>0?1:0, false);
                  this->placeQuote(event, session, requestList, tpnow(), this->exchangePositionUSD_a>0?aSellLevelTakerA:aBuyLevelTakerA, amountTaker_a, productId=="prodA"?0:1, this->exchangePositionUSD_a>0?0:1, false);
              ...
              if(productId=="prodB" && !this->isLiquidationOrderSent_b) {
                ...
                if(amountTaker_b !=0.0 && !this->isLiquidationOrderSent_b) {
                  ...
                  if(this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER) {
                    // send a sell mkt order -> taker
                    aSellLevelTakerB = std::stod(this->bestBidPrice_b) - this->offset * abs(std::stod(this->bestBidPrice_b) - std::stod(this->bestAskPrice_b)); // sell into the order book == mkt order : BEWARE OF THE DUST...

                    // send a buy mkt order
                    aBuyLevelTakerB = std::stod(this->bestAskPrice_b) + this->offset * abs(std::stod(this->bestBidPrice_b) - std::stod(this->bestAskPrice_b)); // buy into the order book == mkt order : BEWARE OF THE DUST...
                  }
                  // double quoteLevel, double quoteAmount, int leg, int side)
                  // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
                  APP_LOGGER_INFO("##### Liquidate order amount B:" + std::to_string(this->exchangePositionUSD_b)); 
                  this->placeQuote(event, session, requestList, tpnow(), this->exchangePositionUSD_b>0?aSellLevelTakerB:aBuyLevelTakerB, amountTaker_b, productId=="prodA"?0:1, this->exchangePositionUSD_b>0?0:1, false);
                  //this->placeQuote(event, session, requestList, tpnow(), 0.0, amountTaker_a, productId=="prodA"?0:1, this->exchangePositionUSD_a>0?0:1, false);
                }
              } 
            } // END LIQUIDATION
            ...
            if(this->isTarget && this->isAllSubscriptionStarted) {
              ...
              if(productId=="prodA" && !this->isLiquidationOrderSent_a) {
                ...
                // send a taker/market order for leg a
                this->exchangePositionUSD_a = -this->exchangePositionUSD_a; // BEWARE: original position meaning within reinit is quote value...therefore the inverse
                ...
                if(amountTaker_a !=0.0 && !this->isLiquidationOrderSent_a) {
                 ...
                 this->remainingLiquidationQty_a = amountTaker_a;
                  ...
                  this->placeQuote(event, session, requestList, tpnow(), amountTaker_a<0?aSellLevelTakerA:aBuyLevelTakerA, abs(amountTaker_a), productId=="prodA"?0:1, amountTaker_a>0?0:1, false);
                }
              }
              ...
              if(productId=="prodB" && !this->isLiquidationOrderSent_b) {
                ...
                // send a taker/market order for leg b
                this->exchangePositionUSD_b = -this->exchangePositionUSD_b; // BEWARE: original position meaning within reinit is quote value...therefore the inverse
                ...
                if(amountTaker_b !=0.0 && !this->isLiquidationOrderSent_b) {
                  ...
                  this->placeQuote(event, session, requestList, tpnow(), amountTaker_b<0?aSellLevelTakerB:aBuyLevelTakerB, abs(amountTaker_b), productId=="prodA"?0:1, amountTaker_b>0?0:1, false);
            ...
            // rebalance mechanism
            if(!this->isStopLossTriggered && (this->isAllSubscriptionStarted && (!(this->isReinit || this->isTarget) || (this->isLiquidated_a && this->isLiquidated_b)))) {//} || 
...
*/
  ...
  double skewUpperBound=1, skewLowerBound=1;
...
*/
  ...
  bool isReinit{}, isTarget{}, isLiquitation{};
  double initTargetPosition_a{}, initTargetPosition_b{};
 ...
 protected:  
  ...
  //int deltaCrossingTime = 60; // number of sec between to occurences (beware mkt message rate...)
    ...
    if(this->previousSyncMicro + deltaSyncPortfolio < currentTimeMicro) {
      ...
      APP_LOGGER_INFO("#####################################################################################################");
      APP_LOGGER_INFO("##### Sync Exchange position with internal position");
  ...
  virtual void processEventExportPerf(const Event& event, Session* session, const TimePoint& messageTime) {
    ...
    if(this->previousExportMicro+deltaPerfExport<currentTimeMicro) {
      ...
      if (!this->bestBidPrice_a.empty() && !this->bestAskPrice_a.empty() &&
        ...
        pstmt_2->setDouble(6, this->skewLowerBound);
        pstmt_2->setDouble(7, this->skewUpperBound);
...
*/
  ...
  virtual void postProcessMessageMarketDataEventPrivateTrade(const Event& event, Session* session, std::vector<Request>& requestList, const Message& message) {
    ...
    for (const auto& element : message.getElementList()) {
      ...
      if(!isMaker) {
        if(productId=="prodA") {
          if((this->isReinit && this->isLiquidated_a) || (this->isTarget && this->isLiquidated_a) || !(this->isReinit || this->isTarget)) {
          ...
          if(this->isTarget && !this->isLiquidated_a) {
            ...
            if(!this->isContractDenominated_a) {
              ...
              this->remainingLiquidationQty_a = this->remainingLiquidationQty_a + lastExecutedSize * std::stod(this->orderQuantityIncrement_a) * (side == CCAPI_EM_ORDER_SIDE_BUY?-1:1);
            }
            if(this->remainingLiquidationQty_a == 0) {
          ...
          if(this->isTarget && !this->isLiquidated_b) {
            ...
            if(!this->isContractDenominated_b) {
              ...
              this->remainingLiquidationQty_b = this->remainingLiquidationQty_b + lastExecutedSize * std::stod(this->orderQuantityIncrement_b) * (side == CCAPI_EM_ORDER_SIDE_BUY?-1:1);
            }
            if(this->remainingLiquidationQty_b == 0) {
