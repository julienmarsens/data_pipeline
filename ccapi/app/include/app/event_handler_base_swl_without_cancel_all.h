...
class EventHandlerBase : public EventHandler {
 public:
  ...
  bool processEvent(const Event& event, Session* session) override {
    ...
    */
    // production
    const TimePoint& nowTp = UtilTime::now();
...
/*
  ...
  // if reqQuantitySec >= MaxRequestSec or reqQuantityHour >= MaxreqestHour then breach -> stop the onQuote method that triggers request 
  // (in addition to rebalancer/market order which correspond to two request (cancel and buy/sell order)) 
  virtual void checkRequestRatesLimits(const TimePoint& now) {

    const TimePoint& nowTp = UtilTime::now();

    // reset states and previous period start time
    if (nowTp >= this->previousShortTimePeriodTp + std::chrono::seconds(this->shortTimeLimiter)) {
      isShortTimeLimiterLimitBreached = false;
      shortTimeIntervalNumberOfRequest = 0;
      this->previousShortTimePeriodTp = nowTp;
    }

/*
  bool isShortLimiter = true;
  bool isLongLimiter = true;

  // for BITMEX
  int shortTimeLimiter = 1; // number of sec == 1[s]
  int longTimeLimiter = 60; // number of sec == 60[s] == 1 min
  ...
  auto previousShortTimePeriodTp = UtilTime::now();
  auto previousLongTimePeriodTp = UtilTime::now();
...
*/
 ...
 protected:
  ...
  virtual void postProcessMessageMarketDataEventMarketDepth(const Event& event, Session* session, std::vector<Request>& requestList, const Message& message, const TimePoint& messageTime) {
    ...
    if(this->previousSamplingSec != this->currentSamplingSec) {
      ...
      if(!this->isShortTimeLimiterLimitBreached && !this->isLongTimeLimiterLimitBreached) {
        ...
        if ((!this->bestBidPrice_a.empty() && !this->bestAskPrice_a.empty() &&
          ...
          APP_LOGGER_DEBUG("##### DEBUG, marginInVector_0:" + std::to_string(marginInVector_0)); 
          APP_LOGGER_DEBUG("##### DEBUG, marginInVector_1:" + std::to_string(marginInVector_1)); 
          APP_LOGGER_DEBUG("##### DEBUG, marginInvSlope:" + std::to_string(marginInvSlope)); 
          ...
          APP_LOGGER_DEBUG("##### DEBUG, normalizedSignalVector_0:" + std::to_string(this->normalizedSignalVector_0));
          APP_LOGGER_DEBUG("##### DEBUG, normalizedSignalVector_1:" + std::to_string(this->normalizedSignalVector_1));
          APP_LOGGER_DEBUG("##### DEBUG, signalPrice_0:" + std::to_string(signalPrice_0));
          APP_LOGGER_DEBUG("##### DEBUG, signalPrice_1:" + std::to_string(signalPrice_1));            
          ...
          APP_LOGGER_DEBUG("##### DEBUG, theoretical price:" + std::to_string(this->theoreticalPrice));
    ...
    */
          ...
          // keep track of position level E [-nc2l,nc2l]
          ...
          if(this->instrument_type_a != "INVERSE") {
            if(!isContractDenominated_a) {
              this->orderAmount_a = this->typicalOrderSize/midPriceA;
            } else {
              this->orderAmount_a = (this->typicalOrderSize/midPriceA)/std::stod(this->orderQuantityIncrement_a);
            }
          } else {
            if(!isContractDenominated_a) {
              this->orderAmount_a = this->typicalOrderSize;
            } else {
              this->orderAmount_a = this->typicalOrderSize/std::stod(this->orderQuantityIncrement_a);
            }
          }
          ...
          if(this->instrument_type_b != "INVERSE") {
            if(!isContractDenominated_b) {
              this->orderAmount_b = this->typicalOrderSize/midPriceB;
            } else {
              this->orderAmount_b = (this->typicalOrderSize/midPriceB)/std::stod(this->orderQuantityIncrement_b);
            }
          } else {
            if(!isContractDenominated_b) {
              this->orderAmount_b = this->typicalOrderSize;
            } else {
              this->orderAmount_b = this->typicalOrderSize/std::stod(this->orderQuantityIncrement_b);
            }
          }*/
          ...
          APP_LOGGER_DEBUG("##########################################################");
    /*
          APP_LOGGER_DEBUG("# [LINEAR_a] std::to_string(this->baseBalance_a + this->orderAmount_a): " + std::to_string(this->baseBalance_a + this->orderAmount_a));
          APP_LOGGER_DEBUG("# [LINEAR_a] std::to_string(this->baseBalance_a - this->orderAmount_a): " + std::to_string(this->baseBalance_a - this->orderAmount_a));

          APP_LOGGER_DEBUG("# [LINEAR_b] std::to_string(this->baseBalance_b + this->orderAmount_b): " + std::to_string(this->baseBalance_b + this->orderAmount_b));
          APP_LOGGER_DEBUG("# [LINEAR_b] std::to_string(this->baseBalance_b - this->orderAmount_b): " + std::to_string(this->baseBalance_b - this->orderAmount_b));

          APP_LOGGER_DEBUG("##########################################################");
          APP_LOGGER_DEBUG("# verbose conditions...");
          APP_LOGGER_DEBUG("# [LINEAR_a] - (this->baseBalance_a + this->orderAmount_a) * midPriceA : " + std::to_string((this->baseBalance_a + this->orderAmount_a) * midPriceA));
          APP_LOGGER_DEBUG("# [LINEAR_a] - abs(this->baseBalance_a - this->orderAmount_a) * midPriceA : " + std::to_string(abs(this->baseBalance_a - this->orderAmount_a) * midPriceA));

          APP_LOGGER_DEBUG("# [INVERSE_a] - (this->quoteBalance_a + this->orderAmount_a) : " + std::to_string((this->quoteBalance_a + this->orderAmount_a)));
          APP_LOGGER_DEBUG("# [INVERSE_a] - abs(this->quoteBalance_a - this->orderAmount_a) : " + std::to_string(abs(this->quoteBalance_a - this->orderAmount_a)));

          APP_LOGGER_DEBUG("# [LINEAR_b] - (this->baseBalance_b + this->orderAmount_b) * midPriceB : " + std::to_string((this->baseBalance_b + this->orderAmount_b) * midPriceB));
          APP_LOGGER_DEBUG("# [LINEAR_b] - abs(this->baseBalance_b - this->orderAmount_b) * midPriceB : " + std::to_string(abs(this->baseBalance_b - this->orderAmount_b) * midPriceB));

          APP_LOGGER_DEBUG("# [INVERSE_b] - (this->quoteBalance_b + this->orderAmount_b) : " + std::to_string((this->quoteBalance_b + this->orderAmount_b)));
          APP_LOGGER_DEBUG("# [INVERSE_b] - abs(this->quoteBalance_b - this->orderAmount_b) : " + std::to_string(abs(this->quoteBalance_b - this->orderAmount_b)));
          APP_LOGGER_DEBUG("##########################################################");
    */
...
*/
          ...
          if(this->instrument_type_a != "INVERSE") {
            //if(!isContractDenominated_a) {
              upperLimitReached_a = -maxInventory_a >= this->quoteBalance_a - this->orderAmount_a * midPriceA;
              lowerLimitReached_a = maxInventory_a <= this->quoteBalance_a + this->orderAmount_a * midPriceA;
            ...
            /*
              ...
              upperLimitReached_a = -maxInventory_a >= this->quoteBalance_a - this->orderAmount_a * midPriceA * std::stod(this->orderQuantityIncrement_a);
              lowerLimitReached_a = maxInventory_a <= this->quoteBalance_a + this->orderAmount_a * midPriceA * std::stod(this->orderQuantityIncrement_a);

              APP_LOGGER_DEBUG("# [prod_a] -> Lower new exposure : " + std::to_string(this->quoteBalance_a + this->orderAmount_a * midPriceA * std::stod(this->orderQuantityIncrement_a)));
              APP_LOGGER_DEBUG("# [prod_a] -> Upper new exposure : " + std::to_string(this->quoteBalance_a - this->orderAmount_a * midPriceA * std::stod(this->orderQuantityIncrement_a)));
            }
            */
          } else {
            //if(!isContractDenominated_a) {
              //upperLimitReached_a = -maxInventory_a >= this->quoteBalance_a - (this->orderAmount_a * this->normalizedTradingVector_0);
              //lowerLimitReached_a = maxInventory_a <= this->quoteBalance_a + (this->orderAmount_a * this->normalizedTradingVector_0);

              upperLimitReached_a = -maxInventory_a >= -this->baseBalance_a * midPriceA - this->orderAmount_a;
              lowerLimitReached_a = maxInventory_a <= -this->baseBalance_a * midPriceA + this->orderAmount_a;

              // APP_LOGGER_DEBUG("# [prod_a] -> Lower new exposure : " + std::to_string(this->quoteBalance_a + (this->orderAmount_a * this->normalizedTradingVector_0)));
              // APP_LOGGER_DEBUG("# [prod_a] -> Upper new exposure : " + std::to_string(this->quoteBalance_a - (this->orderAmount_a * this->normalizedTradingVector_0)));
            ...
            /*
              ...
              //upperLimitReached_a = -maxInventory_a >= this->quoteBalance_a - (this->orderAmount_a * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_a);
              //lowerLimitReached_a = maxInventory_a <= this->quoteBalance_a + (this->orderAmount_a * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_a);  

              upperLimitReached_a = -maxInventory_a >= -this->baseBalance_a * midPriceA - this->orderAmount_a * std::stod(this->orderQuantityIncrement_a);
              lowerLimitReached_a = maxInventory_a <= -this->baseBalance_a * midPriceA + this->orderAmount_a * std::stod(this->orderQuantityIncrement_a);  

              //APP_LOGGER_DEBUG("# [prod_a] -> Lower new exposure : " + std::to_string(this->quoteBalance_a + (this->orderAmount_a * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_a)));
              //APP_LOGGER_DEBUG("# [prod_a] -> Upper new exposure : " + std::to_string(this->quoteBalance_a - (this->orderAmount_a * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_a)));       

              APP_LOGGER_DEBUG("# [prod_a] -> Lower new exposure : " + std::to_string(-this->baseBalance_a * midPriceA + this->orderAmount_a * std::stod(this->orderQuantityIncrement_a)));
              APP_LOGGER_DEBUG("# [prod_a] -> Upper new exposure : " + std::to_string(-this->baseBalance_a * midPriceA - this->orderAmount_a * std::stod(this->orderQuantityIncrement_a)));       
            }
            */
          }
          ...
          if(this->instrument_type_b != "INVERSE") {
            //if(!isContractDenominated_b) {
              upperLimitReached_b = -maxInventory_b >= this->quoteBalance_b - this->orderAmount_b * midPriceB;
              lowerLimitReached_b = maxInventory_b <= this->quoteBalance_b + this->orderAmount_b * midPriceB;
            ...
            /*
              ...
              upperLimitReached_b = -maxInventory_b >= this->quoteBalance_b - this->orderAmount_b * midPriceB * std::stod(this->orderQuantityIncrement_b);
              lowerLimitReached_b = maxInventory_b <= this->quoteBalance_b + this->orderAmount_b * midPriceB * std::stod(this->orderQuantityIncrement_b);

              APP_LOGGER_DEBUG("# [prod_b] -> Lower new exposure : " + std::to_string(this->quoteBalance_b + this->orderAmount_b * midPriceB * std::stod(this->orderQuantityIncrement_b)));
              APP_LOGGER_DEBUG("# [prod_b] -> Upper new exposure : " + std::to_string(this->quoteBalance_b - this->orderAmount_b * midPriceB * std::stod(this->orderQuantityIncrement_b)));
            }
            */
          } else {
            //if(!isContractDenominated_b) {
              //upperLimitReached_b = -maxInventory_b >= this->quoteBalance_b - (this->orderAmount_b * this->normalizedTradingVector_0);
              //lowerLimitReached_b = maxInventory_b <= this->quoteBalance_b + (this->orderAmount_b * this->normalizedTradingVector_0);

              upperLimitReached_b = -maxInventory_b >= -this->baseBalance_b * midPriceB - this->orderAmount_b;
              lowerLimitReached_b = maxInventory_b <= -this->baseBalance_b * midPriceB + this->orderAmount_b;

              // APP_LOGGER_DEBUG("# [prod_b] -> Lower new exposure : " + std::to_string(this->quoteBalance_b + (this->orderAmount_b * this->normalizedTradingVector_0)));
              // APP_LOGGER_DEBUG("# [prod_b] -> Upper new exposure : " + std::to_string(this->quoteBalance_b - (this->orderAmount_b * this->normalizedTradingVector_0)));
            ...
            /*
              ...
              //upperLimitReached_b = -maxInventory_b >= this->quoteBalance_b - (this->orderAmount_b * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_b);
              //lowerLimitReached_b = maxInventory_b <= this->quoteBalance_b + (this->orderAmount_b * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_b);  

              upperLimitReached_b = -maxInventory_b >= -this->baseBalance_b * midPriceB - this->orderAmount_b * std::stod(this->orderQuantityIncrement_b);
              lowerLimitReached_b = maxInventory_b <= -this->baseBalance_b * midPriceB + this->orderAmount_b * std::stod(this->orderQuantityIncrement_b);  

              //APP_LOGGER_DEBUG("# [prod_b] -> Lower new exposure : " + std::to_string(this->quoteBalance_b + (this->orderAmount_b * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_b)));
              //APP_LOGGER_DEBUG("# [prod_b] -> Upper new exposure : " + std::to_string(this->quoteBalance_b - (this->orderAmount_b * this->normalizedTradingVector_0) * std::stod(this->orderQuantityIncrement_b)));       

              APP_LOGGER_DEBUG("# [prod_b] -> Lower new exposure : " + std::to_string(-this->baseBalance_b * midPriceB + this->orderAmount_b * std::stod(this->orderQuantityIncrement_b)));
              APP_LOGGER_DEBUG("# [prod_b] -> Upper new exposure : " + std::to_string(-this->baseBalance_b * midPriceB - this->orderAmount_b * std::stod(this->orderQuantityIncrement_b)));       
            }
            */
          }
          ...
          if((this->aBuyLevelA != this->anOldBuyLevelA || (this->aBuyLevelA != -1 && this->numOpenOrdersBuy_a==0)) && (!isPartialFillBuyOrderA || (isPartialFillBuyOrderA && this->TTLopenBuyOrder_a < 0))) {
          //if((this->aBuyLevelA != this->anOldBuyLevelA || (this->aBuyLevelA != -1 && this->numOpenOrdersBuy_a==0))) {      
            // cancel previous orders for leg A
            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("# Cancel Order prodA, side==buy");
            ...
            if(!upperLimitReached_a) {
              ...
              this->placeQuote(event, session, requestList, messageTimeReceived, this->aBuyLevelA, this->orderAmount_a, 0, 0); // place ONE QUOTE, therefore one more open order
            } else {
              APP_LOGGER_DEBUG("# Risk limit reached, don't place quote A, side==buy");
            }
          }
          
          if((this->aSellLevelA != this->anOldSellLevelA || (this->aSellLevelA != -1 && this->numOpenOrdersSell_a==0)) && (!isPartialFillSellOrderA || (isPartialFillSellOrderA && this->TTLopenSellOrder_a < 0))) {
          //if((this->aSellLevelA != this->anOldSellLevelA || (this->aSellLevelA != -1 && this->numOpenOrdersSell_a==0))) {
            // cancel previous orders for leg A
            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("# Cancel Order prodA, side==sell");
            ...
            if(!lowerLimitReached_a) {
              ...
              this->placeQuote(event, session, requestList, messageTimeReceived, this->aSellLevelA, this->orderAmount_a, 0, 1); // place ONE QUOTE, therefore one more open order
            } else {
              APP_LOGGER_DEBUG("# Risk limit reached, don't place quote A, side==sell");
            }
          }

          if((this->aBuyLevelB != this->anOldBuyLevelB || (this->aBuyLevelB != -1 && this->numOpenOrdersBuy_b==0)) && (!isPartialFillBuyOrderB || (isPartialFillBuyOrderB && this->TTLopenBuyOrder_b < 0))) {
            //if((this->aBuyLevelB != this->anOldBuyLevelB || (this->aBuyLevelB != -1 && this->numOpenOrdersBuy_b==0))) {
            // cancel previous orders for leg B
            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("# Cancel Order prodB, side==buy");
            ...
            if(!upperLimitReached_b) {
              ...
              this->placeQuote(event, session, requestList, messageTimeReceived, this->aBuyLevelB, this->orderAmount_b, 1, 0); // place ONE QUOTE, therefore one more open order
            } else {
              APP_LOGGER_DEBUG("# Risk limit reached, don't place quote B, side==buy");
            }
          }

          if((this->aSellLevelB != this->anOldSellLevelB || (this->aSellLevelB != -1 && this->numOpenOrdersSell_b==0)) && (!isPartialFillSellOrderB || (isPartialFillSellOrderB && this->TTLopenSellOrder_b < 0))) {
            //if((this->aSellLevelB != this->anOldSellLevelB || (this->aSellLevelB != -1 && this->numOpenOrdersSell_b==0))) {
            // cancel previous orders for leg B
            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("# Cancel Order prodB, side==sell");
            ...
            if(!lowerLimitReached_b) {
              ...
              this->placeQuote(event, session, requestList, messageTimeReceived, this->aSellLevelB, this->orderAmount_b, 1, 1); // place ONE QUOTE, therefore one more open order
            } else {
  ...
  virtual void stopLossLimitCheck(const Event& event, Session* session, std::vector<Request>& requestList, const TimePoint& messageTime, double midPriceA, double midPriceB) {
    ...
    if ((this->totalBalancePeak - totalBalance) > this->killSwitchMaximumDrawdown) {
      ...
      */
      ...
      if(inventoryUSD_A>0) {
        // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
        this->placeQuote(event, session, requestList, messageTime, aSellLevelTakerA, abs(this->amountTaker), 0, 1);
      } else {
        this->placeQuote(event, session, requestList, messageTime, aBuyLevelTakerA, abs(this->amountTaker), 0, 0);
      }
      ...
      */
      ...
      if(inventoryUSD_B>0) {
        // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
        this->placeQuote(event, session, requestList, messageTime, aSellLevelTakerB, abs(this->amountTaker), 1, 1);
      } else {
        this->placeQuote(event, session, requestList, messageTime, aBuyLevelTakerB, abs(this->amountTaker), 1, 0);
      }
      ...
      if(productId=="prodA") {
        ...
        */
        ...
        if(ms>0) {
          ...
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, messageTimeReceived, aBuyLevelTakerB, this->amountTaker, 1, 0);
        } else {
          // Sell asset B
          // before submitting a new quote/order make sure that a previous one is not pending...
          ...
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, messageTimeReceived, aSellLevelTakerB, this->amountTaker, 1, 1);
        }
      } else {
        ...
        */
        ...
        if(ms>0) {
          ...
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, messageTimeReceived, aBuyLevelTakerA, this->amountTaker, 0, 0);
        } else {
          // Sell asset B
          // before submitting a new quote/order make sure that a previous one is not pending...
          ...
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, messageTimeReceived, aSellLevelTakerA, this->amountTaker, 0, 1);
        }
...
/*
  ...
  */
  ...
  // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
  virtual void placeQuote(const Event& event, Session* session, std::vector<Request>& requestList, const TimePoint& now, double quoteLevel, double quoteAmount, int leg, int side) {
...
*/
    ...
    if(std::stod(quoteAmount_std) >= 0) {
      ...
      Request request = this->createRequestForCreateOrder2L(side==0?CCAPI_EM_ORDER_SIDE_BUY:CCAPI_EM_ORDER_SIDE_SELL, quoteLevel_std, quoteAmount_std, now, leg);
      requestList.emplace_back(std::move(request));
    }
    //} else {
  ...
  */
       ...
       // we don't check if there is sufficient balance
        ...
        if (productId=="prodA"?this->openBuyOrder_a:this->openBuyOrder_b) {
          ...
          elementList.emplace_back(std::move(element));
          // set to none within order cancel response...duplicate init order state -> flag: JFACANCEL
          /*
          if(productId=="prodA"){
            this->openBuyOrder_a = boost::none;
          } else {
