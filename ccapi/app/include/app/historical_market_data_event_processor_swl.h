 ...
 public:
  ...
  void processEvent() {
    ...
    while (currentDateTp < this->historicalMarketDataEndDateTp) {
      ...
      APP_LOGGER_DEBUG("Start processing " + currentDateISO + ".");
      ...
      //                                   this->instrument_b + "__" +
      //                                   currentDateISO;
      std::cout << "processing fie " << fileNameWithDirBase << std::endl;
      ...
      APP_LOGGER_DEBUG("Opening file " + fileNameWithDirBase + this->historicalMarketDataFileSuffix + ".csv.");

      if (fMarketDepthTrade) {
        APP_LOGGER_DEBUG("Opened file " + fileNameWithDirBase + this->historicalMarketDataFileSuffix + ".csv.");
        ...
        while (std::getline(fMarketDepthTrade, lineMarketDepthTrade) && !lineMarketDepthTrade.empty()) {
          APP_LOGGER_DEBUG("File market-depth & trade next line is " + lineMarketDepthTrade + ".");
        ...
        APP_LOGGER_DEBUG("Warning: unable to open file for date " + UtilTime::getISOTimestamp(currentDateTp));
      } // end open daily file
    APP_LOGGER_DEBUG("End processing " + currentDateISO + ".");
