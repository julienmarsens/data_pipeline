  ...
  virtual void onInit(Session* session) {
    ...
    */
    ...
    if(!isStateExist) {
      ...
      pstmt_3 = con->prepareStatement("INSERT INTO states(TraderId, BaseBalanceA, BaseBalanceB, QuoteBalanceA, QuoteBalanceB, TheoreticalPrice) VALUES (?,?,?,?,?,?)");
    ...
    if(isStateExist && this->isReinit) {
      // is state exists for that trader then reinit it.
      pstmt_3 = con->prepareStatement("UPDATE states SET BaseBalanceA=?, BaseBalanceB=?, QuoteBalanceA=?, QuoteBalanceB=?, TheoreticalPrice=? WHERE TraderId='"+this->trader_id+"';");     
    ...
    pstmt_2 = con->prepareStatement("UPDATE states SET BaseBalanceA=?, BaseBalanceB=?, QuoteBalanceA=?, QuoteBalanceB=?, TheoreticalPrice=? WHERE TraderId='"+this->trader_id+"';");

    if(this->isMysqlexport) {
      // transactions management
      pstmt = con->prepareStatement("INSERT INTO overview(TraderId, BaseBalanceA, BaseBalanceB, QuoteBalanceA, QuoteBalanceB, BestBidPriceA, BestAskPriceA, BestBidPriceB, BestAskPriceB, TotalBalanceA, TotalBalanceB, TotalBalance) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
    }
...
//long previousCrossingMicro{}; = getUnixTimestampMicro(tpnow());
    ...
    */
    ...
    if(this->isShortLimiter ) {
      ...
      APP_LOGGER_DEBUG("#### Previous Unix + delta long time:" + std::to_string(UtilTime::getUnixTimestamp((this->previousShortTimePeriodTp) + std::chrono::seconds(this->shortTimeLimiter))));
    ...
    */
    ...
    if (this->openBuyOrder_a) {
      APP_LOGGER_INFO("Open buy orderA is " + this->openBuyOrder_a->toString() + ".");
    }
    if (this->openSellOrder_a) {
      APP_LOGGER_INFO("Open sell orderA is " + this->openSellOrder_a->toString() + ".");
    }

    if (this->openBuyOrder_b) {
      APP_LOGGER_INFO("Open buy orderB is " + this->openBuyOrder_b->toString() + ".");
    }
    if (this->openSellOrder_b) {
      APP_LOGGER_INFO("Open sell orderB is " + this->openSellOrder_b->toString() + ".");
    }
    ...
    // resync portfolio or check internal positions versus exchange positions (only if all subscriptions are started)
    if(this->isAllSubscriptionStarted || this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER) {
      this->processEventSyncPortfolio(event, session, requestList, tpnow());
    }
    ...
    if (eventType == Event::Type::SUBSCRIPTION_DATA) {
        ...
        if (message.getType() == Message::Type::MARKET_DATA_EVENTS_MARKET_DEPTH && message.getRecapType() == Message::RecapType::NONE) {
          ...
          for (const auto& element : message.getElementList()) {
            ...
            if (this->tradingMode == TradingMode::LIVE) {
              ...
              if(!this->isReinit || (this->isLiquidated_a && this->isLiquidated_b)) {
          ...
          if (!this->privateDataOnlySaveFinalSummary && this->privateTradeCsvWriter && isCSVexport) {
            ...
            for (const auto& element : message.getElementList()) {
              ...
              APP_LOGGER_INFO("#####################################################################################");
              APP_LOGGER_INFO("Private trade details");
      ...
      if (index != -1) {
       ...
       // quoter engine is called under a few...conditions
        if(!this->isStopLossTriggered && ((this->isAllSubscriptionStarted && (!this->isReinit || (this->isLiquidated_a && this->isLiquidated_b))) || this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER)) {
          ...
          if (isBuy) {
            if(this->openBuyOrder_a && this->openBuyOrder_a.get().orderId == orderId) {
              ...
              this->openBuyOrder_a = boost::none;
                ...
                APP_LOGGER_DEBUG("#### DON'T INTERNALLY (SHOULD HAVE OCCURED ON THE EXCHANGE THOUGHT) CANCEL BUY ORDER A WITH EXCHANGE ORDER ID: "+ orderId +" FOR NEW ORDER ID:" +  internalOrderId);
              }
            //this->numOpenOrdersBuy_a = 0;
          } else {
            if(this->openSellOrder_a && this->openSellOrder_a.get().orderId == orderId) {
              ...
              this->openSellOrder_a = boost::none;
                ...
                APP_LOGGER_DEBUG("#### DON'T INTERNALLY (SHOULD HAVE OCCURED ON THE EXCHANGE THOUGHT) CANCEL SELL ORDER A WITH EXCHANGE ORDER ID: "+ orderId +" FOR NEW ORDER ID:" + internalOrderId );
             }
            //this->numOpenOrdersSell_a = 0;
          }
        } else {
          if (isBuy) {
            if(this->openBuyOrder_b && this->openBuyOrder_b.get().orderId == orderId) {
              ...
              this->openBuyOrder_b = boost::none;
                ...
                APP_LOGGER_DEBUG("#### DON'T INTERNALLY (SHOULD HAVE OCCURED ON THE EXCHANGE THOUGHT) CANCEL BUY ORDER B WITH EXCHANGE ORDER ID: "+ orderId +" FOR NEW ORDER ID:" + internalOrderId );
              }
            //this->numOpenOrdersBuy_b = 0;
          } else {
            if(this->openSellOrder_b && this->openSellOrder_b.get().orderId == orderId) {
              ...
              this->openSellOrder_b = boost::none;
                ...
                APP_LOGGER_DEBUG("#### DON'T INTERNALLY (SHOULD HAVE OCCURED ON THE EXCHANGE THOUGHT) CANCEL SELL ORDER B WITH EXCHANGE ORDER ID: "+ orderId +" FOR NEW ORDER ID:" + internalOrderId );
              }
            //this->numOpenOrdersSell_b = 0;
          }
        }
      ...
      /*
        ...
        if(productId=="prodA") {
          if(!this->isStopLossTriggered) {
              ...
              // don't consider upper limit as already taken care of within onQuote function
              this->upperLimitReached_a = false;
              this->lowerLimitReached_a = false;
            ...
            if(!this->isStopLossTriggered) {
              ...
              // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
                            // don't consider upper limit as already taken care of within onQuote function
              this->upperLimitReached_b = false;
              this->lowerLimitReached_b = false;
        ...
        */
        ...
        if (!this->privateDataOnlySaveFinalSummary && this->accountBalanceCsvWriter &&
            (baseBalanceDecimalNotation != "0" || quoteBalanceDecimalNotation != "0") && isCSVexport) {
          this->accountBalanceCsvWriter->writeRow({
              messageTimeReceivedISO,
              productId,
              baseBalanceDecimalNotation,
        ...
        */
        ...
        APP_LOGGER_DEBUG("###################################################");
        APP_LOGGER_DEBUG("Event::Type::RESPONSE recieved -> GET_ACCOUNT_POSITIONS");
        
        APP_LOGGER_DEBUG("CHECK INTERNAL VERSUS EXCHANGE POSITION/BALANCE!");
        ...
        for (const auto& element : firstMessage.getElementList()) {
          ...
          if(instrument_name==this->instrument_a || instrument_name==this->instrument_b) {
            ...
            if(productId=="prodA") {
              APP_LOGGER_DEBUG("#################################################################################################################"); 
              APP_LOGGER_DEBUG("##### Rebalance OR liquidate product: prodA"); 
              ...
              this->remainingLiquidationQty_a = qty; // in case liquidation order is filled in multiple instances...one has to keep track before assuming the position is liquidated

              APP_LOGGER_DEBUG("##### Exchange quote balance_a in USD: "+ std::to_string(this->exchangePositionUSD_a)); 
              ...
              APP_LOGGER_DEBUG("#############################################################"); 
              APP_LOGGER_DEBUG("##### Rebalance OR liquidate product: prodB"); 
              APP_LOGGER_DEBUG("##### Local Balances "); 
              ...
              APP_LOGGER_DEBUG("##### Exchange quote balance_b in USD: "+ std::to_string(this->exchangePositionUSD_b)); 
              APP_LOGGER_DEBUG("#############################################################"); 
            ...
            // if we reinit the trader conf then liquidate position
            if(this->isReinit && this->isAllSubscriptionStarted) {

              if(productId=="prodA" && !this->isLiquidationOrderSent_a) {
                APP_LOGGER_DEBUG("#############################################################"); 
                APP_LOGGER_DEBUG("##### Liquidation process for leg A"); 
                APP_LOGGER_DEBUG("#############################################################"); 
                ...
                if(amountTaker_a !=0.0 && !this->isLiquidationOrderSent_a) {
                  ...
                  // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
                  APP_LOGGER_DEBUG("##### Liquidate position A:" + std::to_string(this->exchangePositionUSD_a)); 
                  this->placeQuote(event, session, requestList, tpnow(), 0.0, amountTaker_a, productId=="prodA"?0:1, this->exchangePositionUSD_a>0?0:1, false);
                }
              } 

              if(productId=="prodB" && !this->isLiquidationOrderSent_b) {
                APP_LOGGER_DEBUG("#############################################################"); 
                APP_LOGGER_DEBUG("##### Liquidation process for leg B"); 
                APP_LOGGER_DEBUG("#############################################################"); 
                ...
                if(amountTaker_b !=0.0 && !this->isLiquidationOrderSent_b) {
                  ...
                  // double quoteLevel, double quoteAmount, int leg, int side)
                  // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
                  APP_LOGGER_DEBUG("##### Liquidate position B:" + std::to_string(this->exchangePositionUSD_b)); 
                  this->placeQuote(event, session, requestList, tpnow(), 0.0, amountTaker_b, productId=="prodA"?0:1, this->exchangePositionUSD_b>0?0:1, false);
                }
              } 
            } // END LIQUIDATION

            // rebalance mechanism
            if(!this->isStopLossTriggered && (this->isAllSubscriptionStarted && (!this->isReinit || (this->isLiquidated_a && this->isLiquidated_b))) || 
            (!this->isStopLossTriggered && (this->tradingMode == TradingMode::BACKTEST || this->tradingMode == TradingMode::PAPER))) {
              ...
              if(this->isAccountBalanceUpdated_a && this->isAccountBalanceUpdated_b) {
                ...
                if(this->exchangePositionUSD_a!=this->quoteBalance_a) {
                  APP_LOGGER_DEBUG("#############################################################"); 
                  APP_LOGGER_DEBUG("##### WARNING: Mis-alignment in LEG_A between exchange and internal position.");
                  //APP_LOGGER_DEBUG("##### STOPPING TRADER!!!");
                  APP_LOGGER_DEBUG("##### CHECK FOR CONNECTIVITY/INTERNAL ERROR AND EITHER LIQUIDATE POSITIONS OR RESTART TRADER WITH REINIT FLAG.");
                  APP_LOGGER_DEBUG("#############################################################"); 
                ...
                if(this->exchangePositionUSD_b!=this->quoteBalance_b) {
                  APP_LOGGER_DEBUG("#############################################################"); 
                  APP_LOGGER_DEBUG("##### WARNING: Mis-alignment in LEG_B between exchange and internal position.");
                  //APP_LOGGER_DEBUG("##### STOPPING TRADER!!!");
                  APP_LOGGER_DEBUG("##### CHECK FOR CONNECTIVITY/INTERNAL ERROR AND EITHER LIQUIDATE POSITIONS OR RESTART TRADER WITH REINIT FLAG.");
                  APP_LOGGER_DEBUG("#############################################################"); 
                  //this->skipProcessEvent=true;
                ...
                // Rem: As an example a large order the leg with a smaller orderquantity increment is executed as maker through partial fill with partial order size smaller
                // than the other leg order quantity increment then no partial fill is executed on the latter leg...leading to misaligment occurs. 
                ...
                if (!this->bestBidPrice_a.empty() && !this->bestAskPrice_a.empty() &&
                  ...
                  double aSellLevelTakerA = std::stod(this->bestBidPrice_a) - this->offset * abs(std::stod(this->bestBidPrice_a) - std::stod(this->bestAskPrice_a)); // sell into the order book == mkt order : BEWARE OF THE DUST...

                  // send a buy mkt order
                  double aBuyLevelTakerA = std::stod(this->bestAskPrice_a) + this->offset * abs(std::stod(this->bestBidPrice_a) - std::stod(this->bestAskPrice_a)); // buy into the order book == mkt order : BEWARE OF THE DUST...

                  // send a sell mkt order
                  double aSellLevelTakerB = std::stod(this->bestBidPrice_b) - this->offset * abs(std::stod(this->bestBidPrice_b) - std::stod(this->bestAskPrice_b)); // sell into the order book == mkt order : BEWARE OF THE DUST...

                  // send a buy mkt order
                  double aBuyLevelTakerB = std::stod(this->bestAskPrice_b) + this->offset * abs(std::stod(this->bestBidPrice_b) - std::stod(this->bestAskPrice_b)); // buy into the order book == mkt order : BEWARE OF THE DUST...
                  ...
                  if(std::stod(this->orderQuantityIncrement_a)<std::stod(this->orderQuantityIncrement_b)) {
                    ...
                    if(abs(abs(this->quoteBalance_b)-abs(this->quoteBalance_a * this->normalizedTradingVector_1/this->normalizedTradingVector_0))>=std::stod(this->orderQuantityIncrement_b)) {
                      ...
                      APP_LOGGER_DEBUG("#############################################################"); 
                      APP_LOGGER_DEBUG("##### Mismatch if position between leg a and b (most probably due to min order size increment).");
                      APP_LOGGER_DEBUG("##### Rebalance product b (min order size b > min order size a) with a market order.");
                      APP_LOGGER_DEBUG("##### quoteBalance_a: "+std::to_string(this->quoteBalance_a));
                      APP_LOGGER_DEBUG("##### quoteBalance_b: "+std::to_string(this->quoteBalance_b));
                      APP_LOGGER_DEBUG("##### targetPosition_b (taking into account trading vector scaling): "+std::to_string(targetPosition_b));                    
                      APP_LOGGER_DEBUG("##### orderSize_b: "+std::to_string(orderSize_b));       
                      APP_LOGGER_DEBUG("#############################################################"); 
                    ...
                    if(abs(abs(this->quoteBalance_a)-abs(this->quoteBalance_b * this->normalizedTradingVector_0/this->normalizedTradingVector_1))>=std::stod(this->orderQuantityIncrement_a)) {
                      ...
                      APP_LOGGER_DEBUG("#############################################################"); 
                      APP_LOGGER_DEBUG("##### Mismatch if position between leg a and b (most probably due to min order size increment).");
                      APP_LOGGER_DEBUG("##### Rebalance product a (min order size a > min order size b) with a market order.");
                      APP_LOGGER_DEBUG("##### quoteBalance_a: "+std::to_string(this->quoteBalance_a));
                      APP_LOGGER_DEBUG("##### quoteBalance_b: "+std::to_string(this->quoteBalance_b));
                      APP_LOGGER_DEBUG("##### targetPosition_a (taking into account trading vector scaling): "+std::to_string(targetPosition_a));                    
                      APP_LOGGER_DEBUG("##### orderSize_a: "+std::to_string(orderSize_a));       
                      APP_LOGGER_DEBUG("#############################################################"); 
...
[2024-01-10T18:54:32.085259650Z] DEBUG: ##### Rebalance product: prodA
[2024-01-10T18:54:32.085262526Z] DEBUG: ##### Local Balances
[2024-01-10T18:54:32.085266525Z] DEBUG: ##### quoteBalance_a: 820.000000
[2024-01-10T18:54:32.085270025Z] DEBUG: ##### baseBalance_a: -0.018013
[2024-01-10T18:54:32.085273449Z] DEBUG: ##### position_qty_a: -82.000000
[2024-01-10T18:54:32.085276406Z] DEBUG: #############################################################
...
[2024-01-10T18:44:32.989751998Z] DEBUG: ##### Rebalance product: prodB
[2024-01-10T18:44:32.989754823Z] DEBUG: ##### Local Balances
[2024-01-10T18:44:32.989758856Z] DEBUG: ##### quoteBalance_b: -520.000000
[2024-01-10T18:44:32.989762111Z] DEBUG: ##### baseBalance_b: 7.822827
[2024-01-10T18:44:32.989765393Z] DEBUG: ##### position_qty_b: 52.000000
[2024-01-10T18:44:32.989768326Z] DEBUG: #############################################################
...
*/
      ...
      for (const auto& message : event.getMessageList()) {
        if (message.getType() == Message::Type::SUBSCRIPTION_STARTED) {
          ...
          for (const auto& correlationId : message.getCorrelationIdList()) {
            ...
            if(this->is_subscription_started_private_trade_a && this->is_subscription_started_private_trade_b && 
              this->is_subscription_started_trade_a && this->is_subscription_started_trade_b &&
              this->is_subscription_started_order_update_a && this->is_subscription_started_order_update_b &&
              this->is_subscription_started_mkt_depth_a && this->is_subscription_started_mkt_depth_b) {

              APP_LOGGER_INFO("#####################################################################################################");
              APP_LOGGER_INFO("#### ALL SUBSCRIPTION STARTED -> READY FOR TRADING -> UNLOCK ONQUOTE METHOD");
              APP_LOGGER_INFO("#####################################################################################################");
  ...
  bool useGetAccountsToGetAccountBalances{}, useCancelOrderToCancelOpenOrders{}, useWebsocketToExecuteOrder{}, useWeightedMidPrice{},
      ...
      immediatelyPlaceNewOrders{}, adverseSelectionGuardTriggerRocOrderDirectionReverse{},
  ...
  bool is_subscription_started_private_trade_a=false, is_subscription_started_private_trade_b=false, is_subscription_started_order_update_a=false, is_subscription_started_order_update_b=false, 
      is_subscription_started_mkt_depth_a=false, is_subscription_started_mkt_depth_b=false, is_subscription_started_trade_a=false, is_subscription_started_trade_b=false;
  
  // ALGORITHM INPUT PARAMETERS
  double normalizedSignalVector_0{}, normalizedSignalVector_1{}, Q{}, margin{}, stepback{}, typicalOrderSize{}, theoreticalPrice{}, 
         maxInventory_a{}, maxInventory_b{}, typicalTradeSize_a{}, typicalTradeSize_b{}, maxOrderSize_a{}, maxOrderSize_b{},
  ...
  // dbg
  //double cumLargeFill=0.0;
  //
  // scaling factor
  //double Q=10;
  //double maxOrderSize_a=500;
  //double maxOrderSize_b=500;

  //double typicalTradeSize_a=100;
  //double typicalTradeSize_b=100; 

  // USD denominated
  //double maxInventory_a=1000;
  //double maxInventory_b=1000;
...
*/
  ...
  // no market chaser implemented within the current logic
  //int offset= 5;//100;
 ...
 protected:  
  ...
  //int deltaCrossingTime = 60; // number of sec between to occurences (beware mkt message rate...)
    ...
    // at t=0 previousSyncMicro=0 therefore triggered
    if(this->previousSyncMicro + deltaSyncPortfolio < currentTimeMicro) {
      ...
      APP_LOGGER_DEBUG("#####################################################################################################");
      APP_LOGGER_DEBUG("##### Sync Exchange position with internal position -> GET_ACCOUNT_POSITION");
      APP_LOGGER_DEBUG("##### current crossing: "+ messageTimeReceivedISO);

      // init account position update state
      isAccountBalanceUpdated_a=false;
      isAccountBalanceUpdated_b=false;
  ...
  virtual void processEventExportPerf(const Event& event, Session* session, const TimePoint& messageTime) {
    ...
    if(this->previousExportMicro+deltaPerfExport<currentTimeMicro) {
      ...
      APP_LOGGER_INFO("#####################################################################################################");
      APP_LOGGER_INFO("# Export balance for the two legs.");   

      APP_LOGGER_INFO("# Base balance A: " + std::to_string(this->baseBalance_a));   
      APP_LOGGER_INFO("# Base balance B: " + std::to_string(this->baseBalance_b));  

      APP_LOGGER_INFO("# Quote balance A: " + std::to_string(this->quoteBalance_a));  
      APP_LOGGER_INFO("# Quote balance B: " + std::to_string(this->quoteBalance_b));
      APP_LOGGER_INFO("#####################################################################################################");

      APP_LOGGER_INFO("best Bid price a: " + this->bestBidPrice_a);
      APP_LOGGER_INFO("best Ask price a: " + this->bestAskPrice_a );
      APP_LOGGER_INFO("best Bid price b: " + this->bestBidPrice_b );
      APP_LOGGER_INFO("best Ask price b: " + this->bestAskPrice_b );
      APP_LOGGER_INFO("# mid-price A: " + std::to_string(this->midPriceA));   
      APP_LOGGER_INFO("# mid-price B: " + std::to_string(this->midPriceB));  
      ...
      if (!this->bestBidPrice_a.empty() && !this->bestAskPrice_a.empty() &&
        ...
        APP_LOGGER_INFO("# Total balance A: " + std::to_string(totBalanceA));    
        APP_LOGGER_INFO("# Total balance B: " + std::to_string(totBalanceB));    
  ...
  // cap order size to max order, see order book liquidity/depth
  std::tuple<double, double> implementMaxOrder(double orderAmount_a, double orderAmount_b) {

      // global parameters
      //double normalizedSignalVector_0=0.7071068;
      //double normalizedSignalVector_1=-0.7071068;

      //int maxOrderSize_a=600;
      //int maxOrderSize_b=500;
      // end global parameters

      // # max.orders is 75% quantile typical order size and delta is product lot size
      
      double minRatio    = std::min(abs(this->maxOrderSize_a/this->normalizedTradingVector_0),abs(this->maxOrderSize_b/this->normalizedTradingVector_1));
      int whichProduct   = minRatio==abs(this->maxOrderSize_a/this->normalizedTradingVector_0)?0:1;

      double realMaxOrder_a = abs(minRatio * this->normalizedTradingVector_0);
      double realMaxOrder_b = abs(minRatio * this->normalizedTradingVector_1);

      if(abs(whichProduct==0?orderAmount_a:orderAmount_b)>(whichProduct==0?this->maxOrderSize_a:this->maxOrderSize_b)) {

        int sign_a = orderAmount_a>0?1:-1;
        int sign_b = orderAmount_b>0?1:-1;

        orderAmount_a=sign_a * realMaxOrder_a;
        orderAmount_b=sign_b * realMaxOrder_b;

        //orderAmount_a=realMaxOrder_a;
        //orderAmount_b=realMaxOrder_b;
      }

      return  {orderAmount_a, orderAmount_b};
  }
    ...
    */
    ...
    //if(this->previousCrossingMicro+deltaCrossingTime<currentTimeMicro) {
      ...
      //if(!this->isShortTimeLimiterLimitBreached && !this->isLongTimeLimiterLimitBreached) {
        ...
        // dbg
        //if(this->bestBidPrice_a=="52226.5") {
        //  APP_LOGGER_DEBUG("dbg");
        //}
        // end dbg
        ...
        if ((!this->bestBidPrice_a.empty() && !this->bestAskPrice_a.empty() &&
          ...
          */
          ...
          if(FP_ZERO == std::fpclassify(this->theoreticalPrice)) {
            ...
            // set first quote to typical order size as no crossings occured at that point in time (therefore no move of margin and no scaling factors...)
            // for prod A
            if(this->instrument_type_a != "INVERSE") {
              this->orderAmount_a = abs(this->typicalTradeSize_a/this->midPriceA);
            } else {
              this->orderAmount_a = abs(this->typicalTradeSize_a);
            }

            // for prod B
            if(this->instrument_type_b != "INVERSE") {
              this->orderAmount_b = abs(this->typicalTradeSize_b/this->midPriceB);
            } else {
              this->orderAmount_b = abs(this->typicalTradeSize_b);
            }

            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("##### Before first crossing, set default quote amount to typical order size -> order amount");
            APP_LOGGER_DEBUG("##### Rem. not scaled yet to contract value.");
            APP_LOGGER_DEBUG("##### order amount a: " + std::to_string(this->orderAmount_a));
            APP_LOGGER_DEBUG("##### order amount b: " + std::to_string(this->orderAmount_b));
            APP_LOGGER_DEBUG("##########################################################");
          }
          ...
          */
          ...
          bool upwardCrossingCondition = (signalPrice_0 - this->theoreticalPrice ) / this->margin > (1 - this->epsilon);
  ...
  double skewLowerBound=1;*/
          ...
          //if(this->isUpwardCrossing || (this->upperLimitReached_b && upwardCrossingCondition) || (this->lowerLimitReached_a && upwardCrossingCondition)) {
            ...
            */
            ...
            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("# moveTheoPrice: "+ std::to_string(moveTheoPrice));
          ...
          bool downwardCrossingCondition = (this->theoreticalPrice - signalPrice_1 ) / this->margin > (1 - this->epsilon);
          ...
          //if(this->isDownwardCrossing  || (this->lowerLimitReached_b && downwardCrossingCondition) || (this->upperLimitReached_a && downwardCrossingCondition)) {// || this->lowerLimitReached_a || this->upperLimitReached_b) {
 ...
 */           
          ...
          // dbg
          //if(abs(moveTheoPrice)>15) {
          //  APP_LOGGER_DEBUG("# Large Move Theo price: " + std::to_string(moveTheoPrice));
          //}
          // end dbg
    ...
    */
          ...
          if(moveTheoPrice !=0) {
            ...
            // scale next quote proportionally to dispersion (next step would be to apply a grid) 
            // raw.order.org      <- cbind(-Q*dPrice$moveTheoPriceVec,-Q*dPrice$moveTheoPriceVec)%*%diag(normalized.trading.vector)
            //dbg
            //if(moveTheoPrice>0) {
              //moveTheoPrice = 14.37997801;
            //} else {
              //moveTheoPrice = -14.37997801;
            //}
            //end dbg

            double rawOrderAmount_a = -this->Q * moveTheoPrice * this->normalizedTradingVector_0;  
            double rawOrderAmount_b = -this->Q * moveTheoPrice * this->normalizedTradingVector_1;  

            // cap to max order size
            auto [cleanOrderAmount_a, cleanOrderAmount_b] = implementMaxOrder(rawOrderAmount_a,rawOrderAmount_b);

            // dbg
            //double cleanOrderAmount_a = rawOrderAmount_a;
            //double cleanOrderAmount_b = rawOrderAmount_b;
            // end dbg

            APP_LOGGER_DEBUG("##########################################################");
            APP_LOGGER_DEBUG("##### A crossing occured, one resize quote amount proportionnaly to the dispertion");
            APP_LOGGER_DEBUG("# previous order amount a: " + std::to_string(this->orderAmount_a));
            APP_LOGGER_DEBUG("# previous order amount b: " + std::to_string(this->orderAmount_b));
            APP_LOGGER_DEBUG("# this->Q: " + std::to_string(this->Q));
            APP_LOGGER_DEBUG("# raw order amount a (no cap): " + std::to_string(rawOrderAmount_a));
            APP_LOGGER_DEBUG("# raw order amount b (no cap): " + std::to_string(rawOrderAmount_b));
            APP_LOGGER_DEBUG("# clean order amount a: " + std::to_string(cleanOrderAmount_a));
            APP_LOGGER_DEBUG("# clean order amount b: " + std::to_string(cleanOrderAmount_b));
            ...
            APP_LOGGER_DEBUG("# CONTRACTION LIMIT LAYER TO ENSURE BOTH LEGS ARE PROPORTIONNALLY REDUCED IN ORDER TO ENSURE AGGREGATED POSITIONS ARE WITHIN RISK LIMITS.");
            //APP_LOGGER_DEBUG("# Max inventory: " + std::to_string(this->maxInventory));
            APP_LOGGER_DEBUG("# Max inventory A: " + std::to_string(this->maxInventory_a));
            APP_LOGGER_DEBUG("# Max inventory B: " + std::to_string(this->maxInventory_b));

            // if current position + order > riskwall then order = riwall-current position
            // BEWARE: one will have to implement linear version of the functionality below (partial implementation)
            // product A
            //if(this->orderAmount_a>0 && (this->maxInventory_a < this->quoteBalance_a + this->orderAmount_a)) {   
            double contractedOrderAmount_a = cleanOrderAmount_a;
            double contractedOrderAmount_b = cleanOrderAmount_b;
            
            if((this->quoteBalance_a + cleanOrderAmount_a)>0 && (this->maxInventory_a < this->quoteBalance_a + cleanOrderAmount_a)) {   
              APP_LOGGER_DEBUG("# Risk wall breached for order amount a>0 -> capping orders");

              contractedOrderAmount_a = this->maxInventory_a-this->quoteBalance_a;
              contractedOrderAmount_b = std::max(cleanOrderAmount_a * this->normalizedTradingVector_1/this->normalizedTradingVector_0, -this->maxInventory_b-this->quoteBalance_b);

              APP_LOGGER_DEBUG("# orderAmount_a: " + std::to_string(contractedOrderAmount_a));
              APP_LOGGER_DEBUG("# orderAmount_b: " + std::to_string(contractedOrderAmount_b));
            } 
            //if(cleanOrderAmount_a<0 && (this->maxInventory_a <= this->quoteBalance_a + cleanOrderAmount_a)) {
            if((this->quoteBalance_a + cleanOrderAmount_a)<0 && (-this->maxInventory_a > this->quoteBalance_a + cleanOrderAmount_a)) {
              APP_LOGGER_DEBUG("# Risk wall breached for order amount a<0 -> capping orders");

              contractedOrderAmount_a = -this->maxInventory_a-this->quoteBalance_a;
              contractedOrderAmount_b = std::min(cleanOrderAmount_a * this->normalizedTradingVector_1/this->normalizedTradingVector_0, this->maxInventory_b-this->quoteBalance_b);
            ...
            // product B
            //if(cleanOrderAmount_b>0 && (this->maxInventory_b < this->quoteBalance_b + cleanOrderAmount_b)) {    
            if((this->quoteBalance_b + cleanOrderAmount_b)>0 && (this->maxInventory_b < this->quoteBalance_b + cleanOrderAmount_b)) {    
              APP_LOGGER_DEBUG("# Risk wall breached for order amount b>0 -> capping orders");     

              contractedOrderAmount_a = this->maxInventory_b-this->quoteBalance_b;
              contractedOrderAmount_b = std::max(cleanOrderAmount_b * this->normalizedTradingVector_0/this->normalizedTradingVector_1, -this->maxInventory_a-this->quoteBalance_a);
            ...
            if((this->quoteBalance_b + cleanOrderAmount_b)<0 && (-this->maxInventory_b > this->quoteBalance_b + cleanOrderAmount_b)) {
              ...
              contractedOrderAmount_a = -this->maxInventory_b-this->quoteBalance_b;
              contractedOrderAmount_b = std::min(cleanOrderAmount_b * this->normalizedTradingVector_0/this->normalizedTradingVector_1, this->maxInventory_a-this->quoteBalance_a);
            ...
            // for prod A
            if(this->instrument_type_a != "INVERSE") {
              this->orderAmount_a = abs(contractedOrderAmount_a/this->midPriceA);
            } else {
              this->orderAmount_a = abs(contractedOrderAmount_a);
            }

            // for prod B
            if(this->instrument_type_b != "INVERSE") {
              this->orderAmount_b = abs(contractedOrderAmount_b/this->midPriceB);
            } else {
              this->orderAmount_b = abs(contractedOrderAmount_b);
            }

            APP_LOGGER_DEBUG("# updated abs order amount a: " + std::to_string(this->orderAmount_a));
            APP_LOGGER_DEBUG("# updated abs order amount b: " + std::to_string(this->orderAmount_b));
            APP_LOGGER_DEBUG("##########################################################");
          ...
          */
          ...
          APP_LOGGER_DEBUG("# Verbose status -> order amount in absolute value!");
          APP_LOGGER_DEBUG("# baseBalance_a: " + std::to_string(this->baseBalance_a)); 
          APP_LOGGER_DEBUG("# quoteBalance_a: " + std::to_string(this->quoteBalance_a)); 
          ...
          if(this->instrument_type_a != "INVERSE") {
              ...
              */
              this->upperLimitReached_a = -this->maxInventory_a >= -this->baseBalance_a * this->midPriceA - this->orderAmount_a * this->midPriceA;
              this->lowerLimitReached_a = this->maxInventory_a <= -this->baseBalance_a * this->midPriceA + this->orderAmount_a * this->midPriceA;
              
              APP_LOGGER_DEBUG("# [prod_a] -> Lower new exposure : " + std::to_string(-this->baseBalance_a * this->midPriceA - this->orderAmount_a * this->midPriceA));
              APP_LOGGER_DEBUG("# [prod_a] -> Upper new exposure : " + std::to_string(-this->baseBalance_a * this->midPriceA + this->orderAmount_a * this->midPriceA));
              ...
              */
              this->upperLimitReached_a = -this->maxInventory_a >= this->quoteBalance_a - this->orderAmount_a;
              this->lowerLimitReached_a = this->maxInventory_a <= this->quoteBalance_a + this->orderAmount_a;
          ...
          if(this->instrument_type_b != "INVERSE") {
              ...
              */
              this->upperLimitReached_b = -this->maxInventory_b >= -this->baseBalance_b * this->midPriceB - this->orderAmount_b * this->midPriceB;
              this->lowerLimitReached_b = this->maxInventory_b <= -this->baseBalance_b * this->midPriceB + this->orderAmount_b * this->midPriceB;
              
              APP_LOGGER_DEBUG("# [prod_b] -> Lower new exposure : " + std::to_string(-this->baseBalance_b * this->midPriceB - this->orderAmount_b * this->midPriceB));
              APP_LOGGER_DEBUG("# [prod_b] -> Upper new exposure : " + std::to_string(-this->baseBalance_b * this->midPriceB + this->orderAmount_b * this->midPriceB));
              ...
              */
              this->upperLimitReached_b = -this->maxInventory_b >= this->quoteBalance_b - this->orderAmount_b;
              this->lowerLimitReached_b = this->maxInventory_b <= this->quoteBalance_b + this->orderAmount_b;
          ...
          // in order to mitigate latency send cancel all message and then resubmit orders
  ...
  virtual void postProcessMessageMarketDataEventPrivateTrade(const Event& event, Session* session, std::vector<Request>& requestList, const Message& message) {
    ...
    for (const auto& element : message.getElementList()) {
      ...
      if(isMaker && abs(lastExecutedSize)>0) {
        ...
        APP_LOGGER_INFO("# Crossing num: " + std::to_string(crossing_num));  
        ...
        /* //dbg
        if(abs(lastExecutedSize)!=170 && abs(lastExecutedSize)!=17) {
          if(productId=="prodA") {
            if (side == CCAPI_EM_ORDER_SIDE_BUY) {
              cumLargeFill=cumLargeFill+lastExecutedSize;
              APP_LOGGER_INFO("#### Bought prod A qty: " + std::to_string(lastExecutedSize));
            } else {
              cumLargeFill=cumLargeFill+lastExecutedSize;
              APP_LOGGER_INFO("#### Sold prod A qty: " + std::to_string(lastExecutedSize));
            }
          } else {
            if (side == CCAPI_EM_ORDER_SIDE_BUY) {
              APP_LOGGER_INFO("#### Bought prod B qty: " + std::to_string(lastExecutedSize));
              cumLargeFill=cumLargeFill+lastExecutedSize;
            } else {
              cumLargeFill=cumLargeFill+lastExecutedSize;
              APP_LOGGER_INFO("#### Sold prod B qty: " + std::to_string(lastExecutedSize));
            }
          }
            APP_LOGGER_INFO("#### checkpoint exec size: " + std::to_string(lastExecutedSize));
            APP_LOGGER_INFO("#### cumLargeFill: " + std::to_string(cumLargeFill));
        }
        */ //end dbg
        ...
        if(side == CCAPI_EM_ORDER_SIDE_SELL) {
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, tpnow(), productId=="prodA"?aBuyLevelTakerB:aBuyLevelTakerA, this->amountTaker, productId=="prodA"?1:0, 0, false);
          ...
          this->placeQuote(event, session, requestList, tpnow(), productId=="prodA"?aSellLevelTakerB:aSellLevelTakerA, this->amountTaker, productId=="prodA"?1:0, 1, false);
...
*/
    ...
    // update position which is expressed in quote currency for inverse and base currency for linear...prevent deleting accumulated pnl in the other currency respectivelly
    if(productId=="prodA") {
      ...
      if(this->instrument_type_a != "INVERSE") {
        if(!isContractDenominated_a) {
          ...
          //this->quoteBalance_a = -qty * std::stod(this->orderQuantityIncrement_a) * this->midPriceA; // with orderQuantityIncrement expressed in base currency
        }
      } else {
        if(!isContractDenominated_a) {
      ...
      if(this->instrument_type_b != "INVERSE") {
        if(!isContractDenominated_b) {
          ...
          //this->quoteBalance_b = -qty * std::stod(this->orderQuantityIncrement_b) * this->midPriceB; // with orderQuantityIncrement expressed in base currency
        }
      } else {
        if(!isContractDenominated_b) {
...
*/
  ...
  virtual void cancelOpenOrders(const Event& event, Session* session, std::vector<Request>& requestList, const TimePoint& messageTime,
    ...
    //APP_LOGGER_DEBUG("CANCEL FUNCTION CALLED NUMBER OF ORDER A: " + productId=="prodA"?std::to_string(this->numOpenOrders_a):std::to_string(this->numOpenOrders_b));
      APP_LOGGER_DEBUG("#################################################################################################################"); 
    ...
    if(productId=="prodA" && side=="sell") {
      if(this->openSellOrder_a && !this->openSellOrder_a.get().orderId.empty()) {
        if(this->openSellOrder_a.get().orderId != this->lastCancelledOrderIdSell_a) {
          ...
          APP_LOGGER_DEBUG("#### this->openSellOrder_a.get().orderId != this->lastCancelledOrderIdSell_a: TRUE; isLastOrderIdAvailable = true; numOrdersWaitingCancelSell_a: " + std::to_string(this->numOrdersWaitingCancelSell_a));
        } else {
          isLastOrderIdAvailable = false;
          this->numOrdersWaitingCancelSell_a +=1;

          APP_LOGGER_DEBUG("#### this->openSellOrder_a.get().orderId != this->lastCancelledOrderIdSell_a: FALSE; isLastOrderIdAvailable = false; numOrdersWaitingCancelSell_a: " + std::to_string(this->numOrdersWaitingCancelSell_a));
        }
      }
    }
    ...
    if(productId=="prodA" && side=="buy") {
      if(this->openBuyOrder_a && !this->openBuyOrder_a.get().orderId.empty()) {
        if(this->openBuyOrder_a.get().orderId != this->lastCancelledOrderIdBuy_a) {
          ...
          APP_LOGGER_DEBUG("#### this->openBuyOrder_a.get().orderId != this->lastCancelledOrderIdBuy_a: TRUE; isLastOrderIdAvailable = true; numOrdersWaitingCancelBuy_a: " + std::to_string(this->numOrdersWaitingCancelBuy_a));
        } else {
          isLastOrderIdAvailable = false;
          this->numOrdersWaitingCancelBuy_a +=1;

          APP_LOGGER_DEBUG("#### this->openBuyOrder_a.get().orderId != this->lastCancelledOrderIdBuy_a: FALSE; isLastOrderIdAvailable = false;numOrdersWaitingCancelBuy_a: " + std::to_string(this->numOrdersWaitingCancelBuy_a));
        }
      }
    }
    // END PROD A
    ...
    if(productId=="prodB" && side=="sell") {
      if(this->openSellOrder_b && !this->openSellOrder_b.get().orderId.empty()) {
        if(this->openSellOrder_b.get().orderId != this->lastCancelledOrderIdSell_b) {
          ...
          APP_LOGGER_DEBUG("#### this->openSellOrder_b.get().orderId != this->lastCancelledOrderIdSell_b: TRUE; isLastOrderIdAvailable = true; numOrdersWaitingCancelSell_b: " + std::to_string(this->numOrdersWaitingCancelSell_b));
        } else {
          isLastOrderIdAvailable = false;
          this->numOrdersWaitingCancelSell_b +=1;

          APP_LOGGER_DEBUG("#### this->openSellOrder_b.get().orderId != this->lastCancelledOrderIdSell_b: FALSE; isLastOrderIdAvailable = false; numOrdersWaitingCancelSell_b: " + std::to_string(this->numOrdersWaitingCancelSell_b));
        }
      }
    }
    ...
    if(productId=="prodB" && side=="buy") {
      if(this->openBuyOrder_b && !this->openBuyOrder_b.get().orderId.empty()) {
        if(this->openBuyOrder_b.get().orderId != this->lastCancelledOrderIdBuy_b) {
          ...
          APP_LOGGER_DEBUG("#### this->openBuyOrder_b.get().orderId != this->lastCancelledOrderIdBuy_b: TRUE; isLastOrderIdAvailable = true; numOrdersWaitingCancelBuy_b: " + std::to_string(this->numOrdersWaitingCancelBuy_b));
        } else {
          isLastOrderIdAvailable = false;
          this->numOrdersWaitingCancelBuy_b +=1;

          APP_LOGGER_DEBUG("#### this->openBuyOrder_b.get().orderId != this->lastCancelledOrderIdBuy_b: FALSE; isLastOrderIdAvailable = false; numOrdersWaitingCancelBuy_b: " + std::to_string(this->numOrdersWaitingCancelBuy_b));
        }
      }
    }
    // END PROD B
...
*/
  ...
  virtual void getAccountPositions(const Event& event, Session* session, std::vector<Request>& requestList, const TimePoint& messageTime, 
    ...
    if(productId=="prodA") {
      Request request(Request::Operation::GET_ACCOUNT_POSITIONS, this->exchange_a, "",
                     "GET_ACCOUNT_POSITIONS#prodA", this->credential_a);
      ...
      Request request(Request::Operation::GET_ACCOUNT_POSITIONS, this->exchange_b, "",
                     "GET_ACCOUNT_POSITIONS#prodB", this->credential_b);
...
/*
  ...
  */
  ...
  virtual void placeQuote(const Event& event, Session* session, std::vector<Request>& requestList, const TimePoint& now, double quoteLevel, double quoteAmount, int leg, int side, bool isPostOnly) {
    ...
    // leg a
    if(leg==0) {
        APP_LOGGER_DEBUG(this->baseAsset_a + " base balance_a is " + Decimal(UtilString::printDoubleScientific(this->baseBalance_a)).toString() + ", " + this->quoteAsset_a +
                        " quote balance_a is " + Decimal(UtilString::printDoubleScientific(this->quoteBalance_a)).toString() + ".");
        APP_LOGGER_DEBUG("Best bid price_a is " + this->bestBidPrice_a + ", best bid size_a is " + this->newBestBidSize_a + ", best ask price_a is " + this->bestAskPrice_a +
                        ", best ask size_a is " + this->newBestAskSize_a + ".");
    } else {
        APP_LOGGER_DEBUG(this->baseAsset_b + " base balance_b is " + Decimal(UtilString::printDoubleScientific(this->baseBalance_b)).toString() + ", " + this->quoteAsset_b +
                        " quote balance_b is " + Decimal(UtilString::printDoubleScientific(this->quoteBalance_b)).toString() + ".");
        APP_LOGGER_DEBUG("Best bid price_b is " + this->bestBidPrice_b + ", best bid size_b is " + this->newBestBidSize_b + ", best ask price_b is " + this->bestAskPrice_b +
    ...
    if(leg==0) {
      if(!this->isContractDenominated_a) {
        ...
        quoteAmount_std = std::to_string(std::stod(AppUtil::roundInput(quoteAmount, this->orderQuantityIncrement_a, false))/std::stod(this->orderQuantityIncrement_a));
      ...
      if(!this->isContractDenominated_b) {
        ...
        quoteAmount_std = std::to_string(std::stod(AppUtil::roundInput(quoteAmount, this->orderQuantityIncrement_b, false))/std::stod(this->orderQuantityIncrement_b));
...
*/
    ...
    //APP_LOGGER_DEBUG(std::string("Submit a new quote for leg: ") + leg==0?"A":"B" + side==0?" Buy ":" Sell " + std::string(quoteAmount_std) + std::string("@") + std::string(quoteLevel_std));
...
/*
          ...
          // JFA: add direction to return object
           element.insert(CCAPI_EM_POSITION_SIDE, std::string(x["direction"].GetString())=="buy"?CCAPI_EM_POSITION_SIDE_LONG:CCAPI_EM_POSITION_SIDE_SHORT); // -> Direction: buy, sell or zero
          ...
          if(productId=="prodA") {
            ...
            if(this->instrument_type_a != "INVERSE") {
              if(!isContractDenominated_a) {
                ...
                element.insert(CCAPI_EM_POSITION_QUANTITY, Decimal(UtilString::printDoubleScientific(abs(this->baseBalance_a/std::stod(this->orderQuantityIncrement_a)))).toString());
                element.insert(CCAPI_EM_POSITION_SIDE, this->baseBalance_a<0?CCAPI_EM_POSITION_SIDE_LONG:CCAPI_EM_POSITION_SIDE_SHORT);
              }
            } else {
              if(!isContractDenominated_a) {
                ...
                element.insert(CCAPI_EM_POSITION_QUANTITY, Decimal(UtilString::printDoubleScientific(abs(this->quoteBalance_a/std::stod(this->orderQuantityIncrement_a)))).toString());
                element.insert(CCAPI_EM_POSITION_SIDE, this->quoteBalance_a<0?CCAPI_EM_POSITION_SIDE_LONG:CCAPI_EM_POSITION_SIDE_SHORT);
              }
            }
          } else {
            ...
            if(this->instrument_type_b != "INVERSE") {
              if(!isContractDenominated_b) {
                ...
                element.insert(CCAPI_EM_POSITION_QUANTITY, Decimal(UtilString::printDoubleScientific(abs(this->baseBalance_b/std::stod(this->orderQuantityIncrement_b)))).toString());
                element.insert(CCAPI_EM_POSITION_SIDE, this->baseBalance_b<0?CCAPI_EM_POSITION_SIDE_LONG:CCAPI_EM_POSITION_SIDE_SHORT);
              }
            } else {
              if(!isContractDenominated_b) {
                ...
                element.insert(CCAPI_EM_POSITION_QUANTITY, Decimal(UtilString::printDoubleScientific(abs(this->quoteBalance_b/std::stod(this->orderQuantityIncrement_b)))).toString());
                element.insert(CCAPI_EM_POSITION_SIDE, this->quoteBalance_b<0?CCAPI_EM_POSITION_SIDE_LONG:CCAPI_EM_POSITION_SIDE_SHORT);
              }              
            }
          }
