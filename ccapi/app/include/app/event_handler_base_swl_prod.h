    ...
    if (eventType == Event::Type::SUBSCRIPTION_DATA) {
      ...
      /*
        ...
        */
        ...
        APP_LOGGER_DEBUG("DISABLED CURRENTLY!");
        ...
        for (const auto& element : firstMessage.getElementList()) {
          ...
          if(instrument_name==this->instrument_a || instrument_name==this->instrument_b) {
            ...
            if(productId=="prodA") {
              ...
              APP_LOGGER_DEBUG("##### Exchange quote balance: "+ std::to_string(-qty)); 
              APP_LOGGER_DEBUG("#############################################################"); 
            } else {
              APP_LOGGER_DEBUG("#################################################################################################################"); 
              ...
              APP_LOGGER_DEBUG("##### Exchange quote balance: "+ std::to_string(-qty * std::stod(this->orderQuantityIncrement_b))); 
              APP_LOGGER_DEBUG("#############################################################"); 
            }
            // end position check
...
*/
  ...
  int deltaPerfExport = 9000000; // makes 9 sec + 1 (see < condition) = every 10 sec min a mysql export may occurs
    ...
    //APP_LOGGER_DEBUG("##### this->previousSamplingSec: "+this->previousSamplingSec);
      ...
      if(productId=="prodA") {
        ...
        */
        ...
        if(this->instrument_type_b != "INVERSE") {
          this->amountTaker = abs(ms/this->midPriceB);
        } else {
          this->amountTaker = abs(ms);
        }
        ...
        /*
        if(this->instrument_type_b != "INVERSE") {
          this->amountTaker = std::ceil(abs(ms/midPriceB)/std::stod(this->orderQuantityIncrement_b))*std::stod(this->orderQuantityIncrement_b);
        } else {
            this->amountTaker = std::ceil(abs(ms)/std::stod(this->orderQuantityIncrement_b))*std::stod(this->orderQuantityIncrement_b);
        }
        */
        ...
        if(ms>0) {
          ...
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, tpnow(), aBuyLevelTakerB, this->amountTaker, 1, 0, false);
          ...
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, tpnow(), aSellLevelTakerB, this->amountTaker, 1, 1, false);
        ...
        */
        ...
        if(this->instrument_type_a != "INVERSE") {
          this->amountTaker = abs(ms/this->midPriceA);
        } else {
          this->amountTaker = abs(ms);
        }
        ...
        /*
        if(this->instrument_type_a != "INVERSE") {
          this->amountTaker = std::ceil(abs(ms/midPriceA)/std::stod(this->orderQuantityIncrement_a))*std::stod(this->orderQuantityIncrement_a);
        } else {
          this->amountTaker = std::ceil(abs(ms)/std::stod(this->orderQuantityIncrement_a))*std::stod(this->orderQuantityIncrement_a);
        }
        */
        ...
        if(ms>0) {
          ...
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, tpnow(), aBuyLevelTakerA, this->amountTaker, 0, 0, false);
          ...
          // double quoteLevel, double quoteAmount, int leg, int side)
          // swl quoter, i.e. leg==0 -> leg a, leg==1 -> leg b ## side==0 -> buy, side==1 -> sell
          this->placeQuote(event, session, requestList, tpnow(), aSellLevelTakerA, this->amountTaker, 0, 1, false);
