...
*/
    ...
    if (eventType == Event::Type::SUBSCRIPTION_DATA) {
      ...
      /*
        ...
        if(productId=="prodA") {
          APP_LOGGER_INFO("#### CANCEL_OPEN_ORDERS FOR LEG A");
          this->openBuyOrder_a = boost::none;
          this->openSellOrder_a = boost::none;
          } else {
          APP_LOGGER_INFO("#### CANCEL_OPEN_ORDERS FOR LEG B");
          this->openBuyOrder_b = boost::none;
          this->openSellOrder_b = boost::none;
          }
        ////
...
*/
 ...
 protected:
  ...
  virtual void postProcessMessageMarketDataEventMarketDepth(const Event& event, Session* session, std::vector<Request>& requestList, const Message& message, const TimePoint& messageTime) {
    ...
    std:: string tmp = this->currentSamplingSec;
...
*/
          ...
          // prodB
          if((this->aBuyLevelB != this->anOldBuyLevelB || (this->aBuyLevelB != -1 && this->numOpenOrdersBuy_b==0)) || (this->aSellLevelB != this->anOldSellLevelB || (this->aSellLevelB != -1 && this->numOpenOrdersSell_b==0))) {
          ...
          // if cancel and resubmit per order as opposed to cancel all and resubmit...or order update
          if(false) {
            // Order management under constrains
...
*/
  ...
  virtual void cancelAllOpenOrders(const Event& event, Session* session, std::vector<Request>& requestList, const TimePoint& messageTime,
    ...
    if(productId=="prodA") {
      ...
      // check that at least one order (either buy or sell) exist for the leg A
     ...
     // check that at least one order (either buy or sell) exist for the leg B
