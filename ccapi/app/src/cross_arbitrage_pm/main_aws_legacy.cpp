...
int main(int argc, char** argv) {
  ...
  // check if the trader state has to be re-init or the state used to restart a trader
  if ( argc == 3 ) {
    reinit = argv[2]; 
  }
  ...
  if(UtilString::toLower(reinit)=="reinit") {
    APP_LOGGER_INFO("******** Reinitialize trader (reset init parameters).");
  } else {
    APP_LOGGER_INFO("******** Restarting trader with previous states.");
  }
  APP_LOGGER_INFO("*********************************************************");
  ...
  std::string accountID_a{}, accountID_b{}, apiKey_a{}, apiKey_b{}, apiSecret_a{}, apiSecret_b{}, baseAssetOverride_a{}, baseAssetOverride_b{}, quoteAssetOverride_a{}, quoteAssetOverride_b{};
  ...
  std::string normalizedSignalVector_0{}, normalizedSignalVector_1{}, normalizedTradingVector_0{}, normalizedTradingVector_1{}, Q{}, margin{}, stepback{}, typicalTrade_0{}, typicalTrade_1{}, maxInventory_0{}, maxInventory_1{};
  std::string maxOrderSize_0{}, maxOrderSize_1{};
...
# ALGORITHM SPECIFIC CONFIGURATION
################################################################
# 1. NORMALIZED SIGNAL VECTOR
NORMALIZED_SIGNAL_VECTOR_0=NA
NORMALIZED_SIGNAL_VECTOR_1=NA

# 2. NORMALIZED TRADING VECTOR
NORMALIZED_TRADING_VECTOR_0=NA
NORMALIZED_TRADING_VECTOR_1=NA
...
# 9. MAX DRAWDOWN STOP LOSS
KILL_SWITCH_MAX_DRAWDOWN=200
  */
  ...
  p->getValue("Q", Q);  
  p->getValue("MAX_ORDER_SIZE_0", maxOrderSize_0);  
  p->getValue("MAX_ORDER_SIZE_1", maxOrderSize_1); 

  p->getValue("MARGIN", margin);   
  p->getValue("STEPBACK", stepback);   

  p->getValue("TYPICAL_TRADE_0", typicalTrade_0);
  p->getValue("TYPICAL_TRADE_1", typicalTrade_1);

  p->getValue("MAX_INVENTORY_0", maxInventory_0);  
  p->getValue("MAX_INVENTORY_1", maxInventory_1); 
  ...
  eventHandler.isReinit =UtilString::toLower(reinit)=="reinit";
  ...
  //if (eventHandler.tradingMode == EventHandlerBase::TradingMode::PAPER || eventHandler.tradingMode == EventHandlerBase::TradingMode::BACKTEST) {
    ...
    eventHandler.Q = std::stod(Q);
    eventHandler.margin = std::stod(margin);
    eventHandler.stepback = std::stod(stepback);
    eventHandler.typicalTradeSize_a = std::stod(typicalTrade_0);
    eventHandler.typicalTradeSize_b = std::stod(typicalTrade_1);         
    eventHandler.maxInventory_a = std::stod(maxInventory_0); 
    eventHandler.maxInventory_b = std::stod(maxInventory_1); 
    eventHandler.maxOrderSize_a = std::stod(maxOrderSize_0); 
    eventHandler.maxOrderSize_b = std::stod(maxOrderSize_1); 
