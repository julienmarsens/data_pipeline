...
class ExecutionManagementServiceOkx : public ExecutionManagementService {
 public:
  ExecutionManagementServiceOkx(std::function<void(Event&, Queue<Event>*)> eventHandler, SessionOptions sessionOptions, SessionConfigs sessionConfigs,
    ...
    //CCAPI_LOGGER_INFO(std::string("##################################### X-Simulated = ") + this->apiXSimulatedTradingName);
    ...
    this->cancelOpenOrdersTarget = "/api/v5/trade/cancel-batch-orders";
  }
  ...
  void convertRequestForRest(http::request<http::string_body>& req, const Request& request, const TimePoint& now, const std::string& symbolId,
    ...
    switch (operation) {
      ...
      case Request::Operation::CANCEL_OPEN_ORDERS: {
        ...
        const auto& paramList = request.getParamList(); //get all params
        ...
        for (size_t i = 0; i < paramList.size(); i += 2) { //add param indice
          rj::Value paramObject(rj::kObjectType);
          // Add instId from entry i
          const auto& instIdMap = paramList[i];
          for (const auto& [key, value] : instIdMap) {
            paramObject.AddMember(
            rj::Value(key.c_str(), allocator),
            rj::Value(value.c_str(), allocator),
            allocator
            );
          }
  
      // Add clOrdId from entry i+1
        const auto& clOrdIdMap = paramList[i + 1]; //add param order id
        for (const auto& [key, value] : clOrdIdMap) {
          paramObject.AddMember(
          rj::Value(key.c_str(), allocator),
          rj::Value(value.c_str(), allocator),
          allocator
        ...
        document.PushBack(paramObject, allocator); //add both params in a json obj
      }
      
      rj::StringBuffer stringBuffer; //doc to string
      rj::Writer<rj::StringBuffer> writer(stringBuffer);
      document.Accept(writer); // Serialize the document to a string
      // Get the JSON string
      auto body = stringBuffer.GetString();
