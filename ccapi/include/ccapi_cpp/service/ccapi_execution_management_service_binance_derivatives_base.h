  ...
  void convertRequestForRest(http::request<http::string_body>& req, const Request& request, const TimePoint& now, const std::string& symbolId,
    ...
    switch (request.getOperation()) {
      case Request::Operation::GET_ACCOUNT_POSITIONS: {
        ...
        const std::map<std::string, std::string> param = request.getFirstParamWithDefault(); //JFA
        //this->appendParam(queryString, {}); //LEGACY

        this->appendParam(queryString, param,
                          {
                              {CCAPI_UNDERLYING_SYMBOL, "pair"},
                          });
    ...
    switch (request.getOperation()) {
      case Request::Operation::GET_ACCOUNT_POSITIONS: {
        for (const auto& x : document.GetArray()) {
          ...
          // don't work see. https://www.reddit.com/r/binance/comments/i3ml0v/question_about_the_futures_api_fapi/
          ...
          element.insert(CCAPI_EM_POSITION_SIDE, std::stod(positionAmt)>0?CCAPI_EM_POSITION_SIDE_LONG:CCAPI_EM_POSITION_SIDE_SHORT);
