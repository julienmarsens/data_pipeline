...
class ExecutionManagementServiceDeribit : public ExecutionManagementService {
 ...
 protected:
  ...
  void signReqeustForRestGenericPrivateRequest(http::request<http::string_body>& req, const Request& request, std::string& httpMethodString,
    ...
    // JFA: prevent duplicated nonce that would lead to reject...
    int64_t nonceInt = std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count();
    std::string nonce = std::to_string(nonceInt);
    //std::string nonce = ts;
  ...
  */
  void signRequest(http::request<http::string_body>& req, const std::string& body, const TimePoint& now, const std::map<std::string, std::string>& credential, const std::string& nonce) {
    std::string authorizationHeader("deri-hmac-sha256 id=");
    authorizationHeader += mapGetWithDefault(credential, this->clientIdName);
    authorizationHeader += ",ts=";
    // JFA
    std::string ts = std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count());
    //std::string ts = this->generateNonce(now, request.getIndex());
  ...
  void prepareReq(http::request<http::string_body>& req, const std::map<std::string, std::string>& param, const TimePoint& now, const std::string& symbolId,
                  ...
                  const std::map<std::string, std::string> standardizationMap = {}, const std::string& nonce={}) {
    ...
    this->signRequest(req, body, now, credential, nonce);
  }
  void convertRequestForRest(http::request<http::string_body>& req, const Request& request, const TimePoint& now, const std::string& symbolId,
    ...
    switch (request.getOperation()) {
      ...
      case Request::Operation::GET_OPEN_ORDERS: {
        const std::map<std::string, std::string> param = request.getFirstParamWithDefault();
        this->prepareReq(req, param, now, symbolId, credential, this->getOpenOrdersTarget, {}, nonce);
      } break;
      case Request::Operation::CANCEL_OPEN_ORDERS: {
        const std::map<std::string, std::string> param = request.getFirstParamWithDefault();
        this->prepareReq(req, param, now, symbolId, credential, this->cancelOpenOrdersTarget, {}, nonce);
      } break;
  ...
  void onTextMessage(wspp::connection_hdl hdl, const std::string& textMessage, const TimePoint& timeReceived) override {
    ...
    //APP_LOGGER_DEBUG("#####################################################################################################");
    //APP_LOGGER_DEBUG("##### TEXT MESSAGE :"+textMessage);
    //APP_LOGGER_DEBUG("#####################################################################################################");
          ...
          if (instrumentSet.find(instrument) != instrumentSet.end()) {
            ...
            if (field == CCAPI_EM_PRIVATE_TRADE && fieldSet.find(CCAPI_EM_PRIVATE_TRADE) != fieldSet.end()) {
              for (const auto& x : params["data"].GetArray()) {
                ...
                // JFA : direction in lower case for deribit...
                //DEBUG        received a text message: {"jsonrpc":"2.0","method":"subscription","params":{"channel":"trades.BTC-PERPETUAL.raw","data":[{"trade_seq":182669516,"trade_id":"264592559","timestamp":1694591012316,"tick_direction":3,"price":26061.5,"mark_price":26069.52,"instrument_name":"BTC-PERPETUAL","index_price":26068.04,"direction":"buy","amount":240.0}]}}
                element.insert(CCAPI_EM_ORDER_SIDE, std::string(x["direction"].GetString()) == "buy" ? CCAPI_EM_ORDER_SIDE_BUY : CCAPI_EM_ORDER_SIDE_SELL);
