...
class MarketDataServiceDeribit : public MarketDataService {
 ...
 private:
  ...
  std::vector<std::string> createSendStringList(const WsConnection& wsConnection) override {
    ...
    //APP_LOGGER_DEBUG("#####################################################################################################");
    //APP_LOGGER_DEBUG(" JFA #####  requestId :"+std::to_string(requestId));
    //APP_LOGGER_DEBUG("#####################################################################################################");
  ...
  void processTextMessage(
    ...
    rj::Document document;
      ...
      //APP_LOGGER_DEBUG("#####################################################################################################");
      //APP_LOGGER_DEBUG(" JFA #####  processTextMessage :"+textMessage);
      //APP_LOGGER_DEBUG("#####################################################################################################");
    ...
    if (it == document.MemberEnd()) {
      ...
      if (method == "subscription") {
        ...
        if (channelId == CCAPI_WEBSOCKET_DERIBIT_CHANNEL_BOOK || channelId == CCAPI_WEBSOCKET_DERIBIT_CHANNEL_QUOTE ||
          ...
          //APP_LOGGER_DEBUG("JFA #####################################################################################################");
          //APP_LOGGER_DEBUG("JFA ##### CCAPI_WEBSOCKET_DERIBIT_CHANNEL_TRADES :");
                
          /*
          for (const auto& x : data.GetArray()) {
            assert(x.IsArray()); // attributes is an array
            for (rapidjson::Value::ConstValueIterator itr = x.Begin(); itr != x.End(); ++itr) {
              const rapidjson::Value& attribute = *itr;
              assert(attribute.IsObject()); // each attribute is an object
                for (rapidjson::Value::ConstMemberIterator itr2 = attribute.MemberBegin(); itr2 != attribute.MemberEnd(); ++itr2) {
                  APP_LOGGER_DEBUG("#####################################################################################################");
                  APP_LOGGER_DEBUG("##### CCAPI_WEBSOCKET_DERIBIT_CHANNEL_TRADES :");
                  APP_LOGGER_DEBUG(itr2->name.GetString());
                  APP_LOGGER_DEBUG( " : ");
                  APP_LOGGER_DEBUG(itr2->value.GetString());
                  APP_LOGGER_DEBUG("#####################################################################################################");
                }
            }
          }
      ...
      //APP_LOGGER_DEBUG("#####################################################################################################");
      //APP_LOGGER_DEBUG(" JFA #####  document.FindMember(id);");
      //APP_LOGGER_DEBUG("#####################################################################################################");
      ...
      if (it != document.MemberEnd()) {
        ...
        if (this->subscriptionJsonrpcIdSetByConnectionIdMap.at(wsConnection.id).find(id) !=
            ...
            //APP_LOGGER_DEBUG("#####################################################################################################");
            //APP_LOGGER_DEBUG(" JFA #####  subscriptionJsonrpcIdSetByConnectionIdMap exists");
            //APP_LOGGER_DEBUG("#####################################################################################################");
